...
export class UsercapacityComponent implements OnInit {
  ...
  fetchTasks(oUser, tasks) {
    for (const index in tasks) {
      if (tasks.hasOwnProperty(index)) {
        ...
        tasks[index].DueDate = this.commonservice.calcTimeForDifferentTimeZone(
  ...
  fetchUserCapacity(oUser, startDate, endDate) {
    const totalAllocatedPerUser = 0,
      totalUnAllocatedPerUser = 0;
    ...
    for (const i in oUser.dates) {
      if (oUser.dates.hasOwnProperty(i)) {
        ...
        if (
          oUser.dates[i].date.getDay() === 6 ||
          oUser.dates[i].date.getDay() === 0
        ) {
        ...
        for (const j in oUser.tasks) {
          if (oUser.tasks.hasOwnProperty(j)) {
            ...
            if (
              oUser.dates[i].date >= taskStartDate &&
              oUser.dates[i].date <= taskEndDate
            ) {
        ...
        const allTimeSpentArray = oUser.TimeSpentTasks.filter(
          (c) => c.TimeSpentDate.getTime() === oUser.dates[i].date.getTime()
        )
  ...
  async fetchTimeSpentTaskDetails(user, date, objt) {
    if (user.TimeSpentTasks.length > 0) {
      const SpentTasks = user.TimeSpentTasks.filter(
        (c) =>
          c.TimeSpentDate.getTime() === date.date.getTime() &&
  ...
  calc(oCapacity) {
    ...
    if (arrDateRange <= 10) {
      setTimeout(() => {
        ...
        tableWidth =
          tableWidth === 0
            ? document.getElementsByClassName("userCapacity")
