...
import { PreStackcommonService } from './service/pre-stackcommon.service';
...
export class PreStackAllocationComponent implements OnInit {
  ...
  constructor(private popupData: DynamicDialogConfig,
    public common: CommonService, public popupConfig: DynamicDialogRef,
  ...
  async checkAllocation(event: any, changedDate: IDailyAllocationObject) {
    ...
    this.allocationSplit = (await this.prestackService.performAllocation(this.resourceCapacity, objData, true,
                                                                         oldValue, oldAllocation, this.allocationSplit)).arrAllocation;
  }
  ...
  //   } else {
  //     const arrAllocation: IPerformAllocationObject = await this.prestackService.performAllocation(resource, allocationData, false, null, null, []);
  ...
  //   const arrTimeSpentPerDay = this.prestackService.getTimeSpentArray(allocationData.strTimeSpent);
  ...
  //       const timeSpent: number = this.prestackService.getTimeSpent(arrTimeSpentPerDay, allocatedDate);
  //       if (allocatedDate) {
  //         let resourceSliderMaxHrs: string = this.prestackService.getResourceMaxHrs(sliderMaxHrs, allocatedDate, index, allocation.value.hours);
