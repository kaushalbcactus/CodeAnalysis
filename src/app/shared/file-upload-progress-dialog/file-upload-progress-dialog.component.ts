...
export class FileUploadProgressDialogComponent implements OnInit {
  ...
  async uploadFiles() {
    ...
    for (let index = 0; index < this.Files.length; index++) {
      const selectedMilestone = this.Files[index].selectedMilestone ? this.Files[index].selectedMilestone : '';
      const filePath = this.config.data.libraryName ?
                       this.config.data.libraryName : this.Files[index].folderPath;
      ...
      this.fileUpload(this.Files[index].file, filePath, this.Files[index].name, this.overwrite).then(uploadedFile => {
        Fileuploadcount++;
        const file = {
          ...uploadedFile as any,
          selectedMilestone
