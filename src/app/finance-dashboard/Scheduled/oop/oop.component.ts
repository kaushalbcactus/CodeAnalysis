...
export class OopComponent implements OnInit, OnDestroy {
  constructor(
    ...
    this.subscription.add(
      this.fdDataShareServie.getScheduleDateRange().subscribe((date) => {
        this.DateRange = date;
  ...
  async getRequiredData() {
    ...
    const obj = Object.assign(
      {},
      isManager
        ? this.fdConstantsService.fdComponent.invoicesOOP
    ...
    for (const element of data) {
      ...
      this.oopBasedRes.push({
        ...
        ScheduledDate: new Date(
          this.datePipe.transform(element.ScheduledDate, "MMM dd, yyyy")
        ), // this.datePipe.transform(element.ScheduledDate, 'MMM dd, yyyy'),
  ...
  confirm1() {
    this.commonService
      .confirmMessageDialog(
        "Confirmation",
        "Are you sure that you want to confirm the invoice scheduled for the project?",
        null,
        ["Yes", "No"],
        false
      )
  ...
  openPopup(data, popUpData) {
    ...
    const projectData = this.projectInfoData.find(
      (e) => e.ProjectCode === data.ProjectCode
    );
    ...
    if (
      ...
      date < lastDay &&
      retPO &&
      new Date(retPO.POExpiryDate) >= todaysDateTimeZero &&
      projectData &&
      projectData.Status !==
        this.constantService.projectList.status.InDiscussion &&
      projectData.Status !==
        this.constantService.projectList.status.AwaitingCancelApproval &&
      ...
      if (
        ...
        this.commonService.showToastrMessage(
          ...
          "Project status is " +
            projectData.Status +
            ", so can not confirm the line item.",
          false
        );
      } else if (!(date >= last3Days && date <= lastDay)) {
        this.commonService.showToastrMessage(
          this.constantService.MessageType.info,
          "To confirm the line item, scheduled Date should be between last 3 working days & last date of the current month.",
          false
        );
      } else if (!retPO) {
        this.commonService.showToastrMessage(
          this.constantService.MessageType.info,
          "PO not available for the selected line item.",
          false
        );
  ...
  openMenuContent(event, data) {
    ...
    if (this.deliverableDialog.title.toLowerCase() === "confirm invoice") {
      ...
      this.deliverableDialog.title.toLowerCase() === "view project details"
    ) {
  ...
  async getApproveExpenseMailContent(type) {
    ...
    const objMailContent = Object.assign(
      {},
      this.fdConstantsService.fdComponent.mailContent
    );
  ...
  async sendCreateExpenseMail() {
    ...
    mailContent = this.replaceContent(
      ...
      this.datePipe.transform(
        this.selectedRowItem.ScheduledDate,
        "MMM dd, yyyy"
      )
    );
    mailContent = this.replaceContent(
      mailContent,
      "@@Val6@@",
      this.selectedRowItem.Currency + " " + this.selectedRowItem.Amount
    );
    mailContent = this.replaceContent(
      mailContent,
      "@@Val7@@",
