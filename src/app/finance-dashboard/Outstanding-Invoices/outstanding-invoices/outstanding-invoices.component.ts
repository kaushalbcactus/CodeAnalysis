...
export class OutstandingInvoicesComponent implements OnInit, OnDestroy {
  ...
  @ViewChild("replaceInvoiceFile", { static: false })
  replaceInvoiceFile: ElementRef;
  @ViewChild("paymentResolvedFile", { static: false })
  paymentResolvedFile: ElementRef;
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      this.outstandingInvoicesRes.push({
        ...
        InvoiceDate: new Date(
          this.datePipe.transform(element.InvoiceDate, "MMM dd, yyyy")
        ),
        InvoiceDateFormat: this.datePipe.transform(
          element.InvoiceDate,
          "MMM dd, yyyy, hh:mm a"
        ),
        InvoiceNumber: element.InvoiceNumber,
        AuxiliaryInvoiceName: element.AuxiliaryInvoiceName
          ? element.AuxiliaryInvoiceName
          : "",
        DisplayInvoiceWithAuxiliary: element.AuxiliaryInvoiceName
          ? element.InvoiceNumber + " - " + element.AuxiliaryInvoiceName
  ...
  createColFieldValues(resArray) {
    this.outInvoiceColArray.ClientLegalEntity = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a) => {
    ...
    this.outInvoiceColArray.InvoiceStatus = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a) => {
            let b = { label: a.InvoiceStatus, value: a.InvoiceStatus };
            return b;
          })
          .filter((ele) => ele.label)
      )
    );
    this.outInvoiceColArray.DisplayInvoiceWithAuxiliary = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a) => {
            let b = {
              label: a.DisplayInvoiceWithAuxiliary,
              value: a.DisplayInvoiceWithAuxiliary,
            };
            return b;
          })
          .filter((ele) => ele.label)
    ...
    this.outInvoiceColArray.PONumber = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a) => {
    ...
    this.outInvoiceColArray.POName = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a) => {
            let b = { label: a.POName, value: a.POName };
            return b;
          })
          .filter((ele) => ele.label)
      )
    );
    this.outInvoiceColArray.Currency = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a) => {
    ...
    this.outInvoiceColArray.POC = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a) => {
    ...
    this.outInvoiceColArray.ModifiedBy = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a) => {
            let b = { label: a.ModifiedBy, value: a.ModifiedBy };
            return b;
          })
          .filter((ele) => ele.label)
      )
    );
    const invoiceDate = this.commonService.sortDateArray(
      this.uniqueArrayObj(
        resArray
          .map((a) => {
            let b = {
              label: this.datePipe.transform(a.InvoiceDate, "MMM dd, yyyy"),
              value: a.InvoiceDate,
            };
            return b;
          })
    ...
    this.outInvoiceColArray.InvoiceDate = invoiceDate
      .map((a) => {
        let b = {
    ...
    this.outInvoiceColArray.Amount = this.fdDataShareServie.customSort(
      amount,
      1,
      "label"
    );
  ...
  openPopup(data, popUpData) {
    ...
    if (data.InvoiceType == "oop" || data.InvoiceType == "revenue") {
      switch (invoiceSts) {
        case "sent to ap": {
          this.items.push(
            ...
            {
              label: "Mark as Payment Resolved",
      ...
      if (
        data.InvoiceStatus === this.constantService.STATUS.GENERATED ||
        data.InvoiceStatus === this.constantService.STATUS.SENTTOAP
      ) {
        this.items.push({
  ...
  openMenuContent(event, data) {
    ...
    if (data.InvoiceType == "oop" || data.InvoiceType == "revenue") {
      if (
        this.confirmDialog.title.toLowerCase() === "mark as payment resolved"
      ) {
      ...
      if (this.confirmDialog.title === "Detag Project") {
        this.commonService
          .confirmMessageDialog(
            "Detag Project",
            "Are you sure you want to detag project",
            null,
            ["Yes", "No"],
            false
          )
  ...
  async OnRowExpand(event) {
    ...
    const url = this.spServices
      .getReadURL(
        this.constantService.listNames.InvoiceLineItems.name,
        this.fdConstantsService.fdComponent.invoiceLineItemByInvoice
      )
      .replace("{{InvoiceLookup}}", event.data.Id);
  ...
  async getFinanceListData() {
    ...
    pfbObj.url = pfbObj.url
      .replace("{{ProjectCode}}", this.selectedRowItem.ProjectCode)
      .replace("{{PO}}", this.selectedRowItem.PO);
  ...
  updateFinanceListData() {
    ...
    let invoiceData = {
      ...
      IsTaggedFully: "No",
      TaggedAmount: this.parentData.TaggedAmount
        ? this.parentData.TaggedAmount - this.selectedRowItem.Amount
        : 0,
    };
    this.commonService.setBatchObject(
      batchUrl,
    ...
    const pfsItems = this.pfresp.filter(
      (pf) => pf.Title === this.selectedRowItem.ProjectCode
    );
    ...
    if (pfsItem) {
      ...
      if (this.selectedRowItem.ScheduleType === "revenue") {
        pfsItem.InvoicedRevenue = pfsItem.InvoicedRevenue
          ? parseFloat(pfsItem.InvoicedRevenue.toFixed(2)) - amount
          : 0;
        pfsItem.ScheduledRevenue = pfsItem.ScheduledRevenue
          ? parseFloat(pfsItem.ScheduledRevenue.toFixed(2)) + amount
          : 0 + amount;
        pfsItem.Invoiced = pfsItem.Invoiced
          ? parseFloat(pfsItem.Invoiced.toFixed(2)) - amount
          : 0;
        pfsItem.InvoicesScheduled = pfsItem.InvoicesScheduled
          ? parseFloat(pfsItem.InvoicesScheduled.toFixed(2)) + amount
          : 0 + amount;
        pfs.push({
          Id: pfsItem.Id,
        ...
        pfsItem.ScheduledOOP = pfsItem.ScheduledOOP
          ? parseFloat(pfsItem.ScheduledOOP.toFixed(2)) + amount
          : 0 + amount;
        pfsItem.InvoicedOOP = pfsItem.InvoicedOOP
          ? parseFloat(pfsItem.InvoicedOOP.toFixed(2)) - amount
          : 0;
        pfsItem.Invoiced = pfsItem.Invoiced
          ? parseFloat(pfsItem.Invoiced.toFixed(2)) - amount
          : 0;
    ...
    pfbsItems = this.pfbresp.filter(
      (pfb) =>
        pfb.ProjectNumber === this.selectedRowItem.ProjectCode &&
        pfb.POLookup === this.selectedRowItem.PO
    );
    if (pfbsItems.length) {
      const amount = parseFloat(this.selectedRowItem.Amount.toFixed(2));
      pfbsItems.forEach((pfbsItem) => {
        if (this.selectedRowItem.ScheduleType === "revenue") {
          pfbsItem.InvoicedRevenue = pfbsItem.InvoicedRevenue
            ? parseFloat(pfbsItem.InvoicedRevenue.toFixed(2)) - amount
            : 0;
          pfbsItem.ScheduledRevenue = pfbsItem.ScheduledRevenue
            ? parseFloat(pfbsItem.ScheduledRevenue.toFixed(2)) + amount
            : 0 + amount;
          pfbsItem.TotalInvoiced = pfbsItem.TotalInvoiced
            ? parseFloat(pfbsItem.TotalInvoiced.toFixed(2)) - amount
            : 0;
          pfbsItem.TotalScheduled = pfbsItem.TotalScheduled
            ? parseFloat(pfbsItem.TotalScheduled.toFixed(2)) + amount
            : 0 + amount;
          pfbs.push({
            Id: pfbsItem.Id,
          ...
          pfbsItem.ScheduledOOP = pfbsItem.ScheduledOOP
            ? parseFloat(pfbsItem.ScheduledOOP.toFixed(2)) + amount
            : 0 + amount;
          pfbsItem.InvoicedOOP = pfbsItem.InvoicedOOP
            ? parseFloat(pfbsItem.InvoicedOOP.toFixed(2)) - amount
            : 0;
          pfbsItem.TotalInvoiced = pfbsItem.TotalInvoiced
            ? parseFloat(pfbsItem.TotalInvoiced.toFixed(2)) - amount
            : 0;
  ...
  onFilePaymentChange(event, folderName) {
    ...
    if (event.target.files && event.target.files.length > 0) {
      ...
      let cleListName = this.getCLEListNameFromCLE(
        this.selectedRowData[0].ClientLegalEntity
      );
      ...
      .then(async (uploadedfile) => {
        if (
          ...
          if (
            ...
            if (type === "creditDebit") {
              this.submitDebitCreditNoteForm(
                type,
              ...
              if (type === "replaceInvoice") {
                ...
                invData = {
                  Status: "Paid",
                  PaymentURL: uploadedfile[0].ServerRelativeUrl
                    ? uploadedfile[0].ServerRelativeUrl
                    : "",
                };
              }
  ...
  onCorDChange(data: any) {
    ...
    if (item.value === "Credit Note") {
      this.creditOrDebitNote_form.controls["Status"].setValue(
        this.constantService.invoicesStatus.AwaitingClosedCreditNote
      );
    } else if (item.value === "Debit Note") {
      this.creditOrDebitNote_form.controls["Status"].setValue(
        this.constantService.invoicesStatus.AwaitingClosedDebitNote
      );
    }
  ...
  async onSubmit(type: string) {
    ...
    if (type === "paymentResoved") {
      ...
      if (
        this.SelectedAuxInvoiceName.trim() === "" ||
        this.SelectedAuxInvoiceName.length > 30
      ) {
        const errorMessage =
          this.SelectedAuxInvoiceName.trim() === ""
            ? "Please enter Auxiliary Name"
  ...
  submitDebitCreditNoteForm(type: string, pathURL) {
    ...
    debitCreditData["__metadata"] = {
      type: this.constantService.listNames.CreditAndDebit.type,
    };
    const debCreditObj = Object.assign({}, this.queryConfig);
    debCreditObj.url = this.spServices.getReadURL(
      this.constantService.listNames.CreditAndDebit.name
    );
  ...
  async submitForm(batchUrl, type: string) {
    ...
    if (type === "creditDebit") {
      this.commonService.showToastrMessage(
        ...
        this.selectedRowItem.InvoiceNumber + " " + "Success.",
        true
      );
      this.creditOrDebitModal = false;
      this.reFetchData();
      ...
      this.commonService.showToastrMessage(
        this.constantService.MessageType.success,
        this.selectedRowItem.InvoiceNumber + " " + "Submitted.",
        true
      );
      this.disputeInvoiceModal = false;
    } else if (type === "paymentResoved") {
      this.commonService.showToastrMessage(
        this.constantService.MessageType.success,
        "Payment receipts uploaded successfully.",
        true
      );
      this.paymentResovedModal = false;
      this.reFetchData();
  ...
  async generateExistingInvoice(lineItem) {
    ...
    prfObj.filter = prfObj.filter.replace(
      "{{ItemID}}",
      lineItem.ProformaLookup
    );
    this.commonService.SetNewrelic(
      "Finance-Dashboard",
      "outstanding-invoices",
      "generateExistingInvoice",
      "GET"
    );
    const res = await this.spServices.readItems(
    ...
    if (proforma.ProformaHtml) {
      ...
      const response = await this.fdDataShareServie.createInvoice(
        proforma.ProformaHtml,
        proforma,
        lineItem,
        this.cleData
      );
  ...
  sentToAP() {
    if (this.selectedRowData.length) {
      ...
      if (!filterData.length) {
        ...
        this.commonService.showToastrMessage(
          this.constantService.MessageType.info,
          "Please select only those Row Item whose Invoices is Generated. ",
          false
        );
      }
    } else {
  ...
  uploadReceipts() {
    if (this.selectedRowData.length) {
      const filterData = this.selectedRowData.filter(
        (e) => e.InvoiceStatus !== "Sent to AP"
      );
      const cle = this.selectedRowData[0].ClientLegalEntity;
      const cleCheck = this.selectedRowData.filter(
        (c) => c.ClientLegalEntity !== cle
      );
      if (!filterData.length && !cleCheck.length) {
        ...
        if (cleCheck.length) {
          this.commonService.showToastrMessage(
            this.constantService.MessageType.info,
            "Please select only those Row Item whose Client is Similar. ",
            false
          );
        } else {
          this.commonService.showToastrMessage(
            this.constantService.MessageType.info,
            "Please select only those Row Item whose Invoices is Sent to AP. ",
            false
          );
        }
      }
