...
export class ProformaComponent implements OnInit, OnDestroy {
  ...
  async getRequiredData() {
    ...
    const res = await this.spServices.readItems(
      this.constantService.listNames.Proforma.name,
      prfObj
    );
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      this.proformaRes.push({
        ...
        ProformaDate: new Date(
          this.datePipe.transform(element.ProformaDate, "MMM dd, yyyy")
        ),
        ProformaDateFormat: this.datePipe.transform(
  ...
  confirm1(obj) {
    this.commonService
      .confirmMessageDialog(obj.title, obj.msg, null, ["Yes", "No"], false)
  ...
  openPopup(data, popUpData, proforma) {
    ...
    if (data.type === "Proforma") {
      ...
      if (
        ...
        this.items.push({
          label: "Regenerate Proforma",
          command: (e) => this.openMenuContent(e, data),
        });
      }

      this.items.push(
        { label: "Edit Poc", command: (e) => this.openMenuContent(e, data) },
        {
          label: "Reject Proforma",
          command: (e) => this.openMenuContent(e, data),
        },
        {
          label: "Replace Proforma",
          command: (e) => this.openMenuContent(e, data),
        }
      );
    } else {
  ...
  openMenuContent(event, data) {
    ...
    if (data.type === "Proforma") {
      if (this.confirmDialog.title.toLowerCase() === "mark as sent to client") {
        ...
        this.proformaConfirmationModal.msg =
          'Are you sure you want to change proforma status to "Sent" ?';
        ...
        this.proformaConfirmationModal.type = this.confirmDialog.title;
        this.proformaConfirmationModal.msg =
          "Are you sure you want to reject this proforma?";
        this.proformaConfirmationModal.title = "Update Proforma";
        this.getILIByPID();
        this.confirm1(this.proformaConfirmationModal);
      } else if (
        this.confirmDialog.title.toLowerCase() === "replace proforma"
      ) {
        this.replaceProformaModal = true;
      } else if (
        this.confirmDialog.title.toLowerCase() === "generate invoice"
      ) {
        ...
        if (proformaObj.hasOwnProperty("saveObj")) {
          this.fdConstantsService.fdComponent.selectedEditObject.Code =
            data.ProformaNumber;
          const oCLE = this.cleData.find(
            (e) => e.Title === data.ClientLegalEntity
          );
          this.fdConstantsService.fdComponent.selectedEditObject.ListName =
            oCLE.ListName;
          this.fdConstantsService.fdComponent.selectedEditObject.ID = data.Id;
          this.fdConstantsService.fdComponent.selectedEditObject.Type =
            "Proforma";
        ...
        ref.onClose.subscribe((editInvoice: any) => {
          if (editInvoice) {
            const batchURL = this.fdDataShareServie.EditInvoiceDialogProcess(
              this.selectedRowItem.ScheduleType,
  ...
  async getILIByPID() {
    this.fdConstantsService.fdComponent.isPSInnerLoaderHidden = false;
    const iliObj = Object.assign(
      {},
      this.fdConstantsService.fdComponent.invoiceLineItem
    );
    iliObj.filter = iliObj.filter.replace(
      "{{ProformaLookup}}",
  ...
  uniqueArrayObj1(array: any) {
    let sts: any = "";
    return (sts = Array.from(new Set(array.map((s) => s.Title))).map(
      (label1) => {
        return {
          Title: label1,
  ...
  async getPfPfbSow(array: any) {
    ...
    for (let j = 0; j < array.length; j++) {
      ...
      pfbObj.url = pfbObj.url
        .replace("{{ProjectCode}}", element.item.Title)
        .replace("{{PO}}", element.item.PO);
      ...
      sowObj.url = this.spServices.getReadURL(
  ...
  async addUpdateRequired() {
    ...
    const cleItem = this.getClEItemByPNum(
      this.selectedRowItem.ClientLegalEntity
    );
    const invDate = this.generateInvoice_form.value.InvoiceDate;
    const mmyy =
      this.datePipe.transform(new Date(invDate), "MM") +
      this.datePipe.transform(new Date(invDate), "yy");
    ...
    cleObj.url = this.spServices.getItemURL(
      this.constantService.listNames.ClientLegalEntity.name,
      +cleItem.Id
    );
    ...
    if (this.selectedRowItem) {
      ...
      // tslint:disable
      this.selectedRowItem.Amount = parseFloat(
        this.selectedRowItem.Amount.toFixed(2)
      );
      if (proformaType === "revenue") {
        if (this.addILIObj.IsTaggedFully === "Yes") {
          poData.ScheduledRevenue = poItem.ScheduledRevenue
            ? parseFloat(poItem.ScheduledRevenue.toFixed(2)) -
              this.selectedRowItem.Amount
            : 0 - this.selectedRowItem.Amount;
          poData.TotalScheduled = poItem.TotalScheduled
            ? parseFloat(poItem.TotalScheduled.toFixed(2)) -
              this.selectedRowItem.Amount
            : 0 - this.selectedRowItem.Amount;
        }
        poData.InvoicedRevenue = poItem.InvoicedRevenue
          ? parseFloat(poItem.InvoicedRevenue.toFixed(2)) +
            this.selectedRowItem.Amount
          : 0 + this.selectedRowItem.Amount;
        poData.TotalInvoiced = poItem.TotalInvoiced
          ? parseFloat(poItem.TotalInvoiced.toFixed(2)) +
            this.selectedRowItem.Amount
          : 0 + this.selectedRowItem.Amount;
      } else if (proformaType === "oop") {
        if (this.addILIObj.IsTaggedFully === "Yes") {
          poData.ScheduledOOP = poItem.ScheduledOOP
            ? parseFloat(poItem.ScheduledOOP.toFixed(2)) -
              this.selectedRowItem.Amount
            : 0 - this.selectedRowItem.Amount;
          poData.TotalScheduled = poItem.TotalScheduled
            ? parseFloat(poItem.TotalScheduled.toFixed(2)) -
        ...
        poData.InvoicedOOP = poItem.InvoicedOOP
          ? parseFloat(poItem.InvoicedOOP.toFixed(2)) +
            this.selectedRowItem.Amount
          : 0 + this.selectedRowItem.Amount;
        poData.TotalInvoiced = poItem.TotalInvoiced
          ? parseFloat(poItem.TotalInvoiced.toFixed(2)) +
    ...
    poObj.url = this.spServices.getItemURL(
    ...
    this.iliByPidRes.forEach((element) => {
      ...
      if (pfsItem) {
        ...
        if (proformaType === "revenue") {
          pfsItem.InvoicedRevenue = pfsItem.InvoicedRevenue
            ? parseFloat(pfsItem.InvoicedRevenue.toFixed(2)) + element.Amount
            : 0 + element.Amount;
          pfsItem.ScheduledRevenue = pfsItem.ScheduledRevenue
            ? parseFloat(pfsItem.ScheduledRevenue.toFixed(2)) - element.Amount
            : 0 - element.Amount;
          pfsItem.Invoiced = pfsItem.Invoiced
            ? parseFloat(pfsItem.Invoiced.toFixed(2)) + element.Amount
            : 0 + element.Amount;
          pfsItem.InvoicesScheduled = pfsItem.InvoicesScheduled
            ? parseFloat(pfsItem.InvoicesScheduled.toFixed(2)) - element.Amount
            : 0 - element.Amount;
          pfs.push({
            Id: pfsItem.Id,
          ...
          pfsItem.ScheduledOOP = pfsItem.ScheduledOOP
            ? parseFloat(pfsItem.ScheduledOOP.toFixed(2)) - element.Amount
            : 0 - element.Amount;
          pfsItem.InvoicedOOP = pfsItem.InvoicedOOP
            ? parseFloat(pfsItem.InvoicedOOP.toFixed(2)) + element.Amount
            : 0 + element.Amount;
          pfsItem.Invoiced = pfsItem.Invoiced
            ? parseFloat(pfsItem.Invoiced.toFixed(2)) + element.Amount
            : 0 + element.Amount;
      ...
      pfbsItems = this.pfbresp.filter(
        (pfb) =>
          pfb.ProjectNumber === element.Title && pfb.POLookup === element.PO
      );
      if (pfbsItems.length) {
        pfbsItems.forEach((pfbsItem) => {
          if (proformaType === "revenue") {
            pfbsItem.InvoicedRevenue = pfbsItem.InvoicedRevenue
              ? parseFloat(pfbsItem.InvoicedRevenue.toFixed(2)) + element.Amount
              : 0 + element.Amount;
            pfbsItem.ScheduledRevenue = pfbsItem.ScheduledRevenue
              ? parseFloat(pfbsItem.ScheduledRevenue.toFixed(2)) -
                element.Amount
              : 0 - element.Amount;
            pfbsItem.TotalInvoiced = pfbsItem.TotalInvoiced
              ? parseFloat(pfbsItem.TotalInvoiced.toFixed(2)) + element.Amount
              : 0 + element.Amount;
            pfbsItem.TotalScheduled = pfbsItem.TotalScheduled
              ? parseFloat(pfbsItem.TotalScheduled.toFixed(2)) - element.Amount
              : 0 - element.Amount;
            pfbs.push({
              Id: pfbsItem.Id,
            ...
            pfbsItem.ScheduledOOP = pfbsItem.ScheduledOOP
              ? parseFloat(pfbsItem.ScheduledOOP.toFixed(2)) - element.Amount
              : 0 - element.Amount;
            pfbsItem.InvoicedOOP = pfbsItem.InvoicedOOP
              ? parseFloat(pfbsItem.InvoicedOOP.toFixed(2)) + element.Amount
              : 0 + element.Amount;
            pfbsItem.TotalInvoiced = pfbsItem.TotalInvoiced
              ? parseFloat(pfbsItem.TotalInvoiced.toFixed(2)) + element.Amount
              : 0 + element.Amount;
      ...
      const sowItems = this.sowresp.filter(
        (pf) => pf.SOWCode === element.SOWCode
      );
      sowsItem = sowItems.length > 0 ? sowItems[0] : {};
      if (sowsItem) {
        if (proformaType === "revenue") {
          sowsItem.InvoicedRevenue = sowsItem.InvoicedRevenue
            ? parseFloat(sowsItem.InvoicedRevenue.toFixed(2)) + element.Amount
            : 0 + element.Amount;
          sowsItem.ScheduledRevenue = sowsItem.ScheduledRevenue
            ? parseFloat(sowsItem.ScheduledRevenue.toFixed(2)) - element.Amount
            : 0 - element.Amount;
          sowsItem.TotalInvoiced = sowsItem.TotalInvoiced
            ? parseFloat(sowsItem.TotalInvoiced.toFixed(2)) + element.Amount
            : 0 + element.Amount;
          sowsItem.TotalScheduled = sowsItem.TotalScheduled
            ? parseFloat(sowsItem.TotalScheduled.toFixed(2)) - element.Amount
            : 0 - element.Amount;
          sows.push({
            Id: sowsItem.Id,
          ...
          sowsItem.ScheduledOOP = sowsItem.ScheduledOOP
            ? parseFloat(sowsItem.ScheduledOOP.toFixed(2)) - element.Amount
            : 0 - element.Amount;
          sowsItem.InvoicedOOP = sowsItem.InvoicedOOP
            ? parseFloat(sowsItem.InvoicedOOP.toFixed(2)) + element.Amount
            : 0 + element.Amount;
          sowsItem.TotalInvoiced = sowsItem.TotalInvoiced
            ? parseFloat(sowsItem.TotalInvoiced.toFixed(2)) + element.Amount
            : 0 + element.Amount;
    ...
    if (pfs.length) {
      for (let pf = 0; pf < pfs.length; pf++) {
        ...
        const pfObj = Object.assign({}, this.queryConfig);
        pfObj.url = this.spServices.getItemURL(
          this.constantService.listNames.ProjectFinances.name,
          +element.Id
        );
        pfObj.listName = this.constantService.listNames.ProjectFinances.name;
    ...
    if (pfbs.length) {
      for (let pfb = 0; pfb < pfbs.length; pfb++) {
        ...
        const pfbObj = Object.assign({}, this.queryConfig);
        pfbObj.url = this.spServices.getItemURL(
          this.constantService.listNames.ProjectFinanceBreakup.name,
          +element.Id
        );
        pfbObj.listName = this.constantService.listNames.ProjectFinanceBreakup.name;
    ...
    if (sows.length) {
      for (let sow = 0; sow < sows.length; sow++) {
        ...
        element["__metadata"] = {
          type: this.constantService.listNames.SOW.type,
        };

        const sowObj = Object.assign({}, this.queryConfig);
        sowObj.url = this.spServices.getItemURL(
          this.constantService.listNames.SOW.name,
          +element.Id
        );
        sowObj.listName = this.constantService.listNames.SOW.name;
  ...
  onFileChange(event, folderName) {
    let existingFile = this.selectedRowItem.FileURL
      ? this.selectedRowItem.FileURL.split("/")
      : [];
    if (existingFile) {
      ...
      if (file === event.target.files[0].name) {
        this.commonService.showToastrMessage(
          ...
          this.constantService.Messages.FileAlreadyExist,
          false
        );
    ...
    if (event.target.files && event.target.files.length > 0) {
      ...
      if (fileName !== sNewFileName) {
        ...
        this.commonService.showToastrMessage(
          this.constantService.MessageType.error,
          this.constantService.Messages.SpecialCharMsg,
          false
        );
        return false;
      }
      let cleListName = this.getCLEListNameFromCLE(
        this.selectedRowItem.ClientLegalEntity
      );
  ...
  async uploadFileData() {
    ...
    this.commonService
      .UploadFilesProgress(this.SelectedFile, this.FolderName, true)
      .then(async (uploadedfile) => {
        if (
          ...
          if (
            ...
            this.commonService.showToastrMessage(
              this.constantService.MessageType.error,
              this.constantService.Messages.FileNotUploaded,
              false
            );
            ...
            const invObj = Object.assign({}, this.queryConfig);
            invObj.url = this.spServices.getItemURL(
              this.constantService.listNames.Proforma.name,
              +this.selectedRowItem.Id
            );
  ...
  onSubmit(type: string) {
    ...
    if (type === "Mark as Sent to Client") {
      ...
      if (this.replaceProforma_form.invalid) {
        ...
        this.commonService.showToastrMessage(
          this.constantService.MessageType.error,
          this.constantService.Messages.ZeroKbFile.replace(
            "{{fileName}}",
  ...
  async submitForm(dataEndpointArray, type: string) {
    ...
    if (type === "Mark as Sent to Client" || type === "Reject Proforma") {
      ...
      this.commonService.showToastrMessage(
        ...
        'Proforma Status changed to "' + sts + '" Successfully.',
        true
      );
      this.reFetchData(type);
    } else if (type === "createProforma") {
      await this.fdDataShareServie.callProformaCreation(
        arrResults[0],
        this.cleData,
        this.projectContactsData,
      ...
      this.commonService.showToastrMessage(
        this.constantService.MessageType.success,
        arrResults[0].Title + " " + "Proforma Created.",
        true
      );
      this.reFetchData(type);
    } else if (type === "generateInvoice") {
      ...
      for (let j = 0; j < this.iliByPidRes.length; j++) {
        ...
        iliObj.url = this.spServices.getItemURL(
          this.constantService.listNames.InvoiceLineItems.name,
          +element.Id
        );
      ...
      proformaData["__metadata"] = {
        type: this.constantService.listNames.Proforma.type,
      };
      const prfObj = Object.assign({}, this.queryConfig);
      prfObj.url = this.spServices.getItemURL(
        this.constantService.listNames.Proforma.name,
        +this.selectedRowItem.Id
      );
      ...
      if (proformHtml) {
        await this.fdDataShareServie.createInvoice(
          proformHtml,
          this.selectedRowItem,
          oInv,
          this.cleData
      ...
      this.commonService.showToastrMessage(
        this.constantService.MessageType.success,
        "Invoice Number: " + oInv.InvoiceNumber,
        true
      );
      this.reFetchData(type);
    } else if (type === "replaceProforma") {
      this.commonService.showToastrMessage(
        this.constantService.MessageType.success,
        this.selectedRowItem.ProformaNumber + " " + "Success.",
        true
      );
      this.replaceProformaModal = false;
      this.reFetchData(type);
  ...
  async reFetchData(type: string) {
    if (
      type === "Mark as Sent to Client" ||
      type === "Reject Proforma" ||
      type === "replaceProforma" ||
      type === "editInvoice" ||
      type === "editPOC"
    ) {
  ...
  async generateExistingProforma(rowItem) {
    ...
    if (this.cleData.find((c) => c.Title === rowItem.ClientLegalEntity)) {
      const res = await this.spServices.checkFileExist(
        this.globalService.sharePointPageObject.webAbsoluteUrl +
          "/" +
          this.cleData.find((c) => c.Title === rowItem.ClientLegalEntity)
            .ListName
      );
      if (res.hasOwnProperty("status")) {
        if (res.status === 404) {
          this.commonService.showToastrMessage(
            this.constantService.MessageType.error,
            rowItem.ProformaNumber + " Proforma not created. Folder Not Found.",
            true
          );
          this.fdConstantsService.fdComponent.isPSInnerLoaderHidden = true;
        } else {
          const response = await this.getILIByPIDForProforma(rowItem.Id);
          const projectAppendix = await this.createProjectAppendix(
            this.projectContactsData,
            response
          );
          ...
          setTimeout(() => {
            this.commonService.showToastrMessage(
              this.constantService.MessageType.success,
              rowItem.ProformaNumber + " Proforma Created.",
              false
            );
          }, 500);
        }
      ...
      this.commonService.showToastrMessage(
        this.constantService.MessageType.error,
        rowItem.ProformaNumber + " Proforma not created. Client Not Found.",
        true
      );
      this.fdConstantsService.fdComponent.isPSInnerLoaderHidden = true;
    }
  ...
  async createProjectAppendix(prjContacts, selectedProjects) {
    ...
    selectedProjects.forEach((element) => {
      if (projectProcessed.indexOf(element.Title) === -1) {
        const project = this.projectInfoData.find(
          (e) => e.ProjectCode === element.Title
        );
        if (project) {
    ...
    if (callProjects.length) {
      ...
      for (const element of callProjects) {
        ...
        getPIData.url = this.spServices.getReadURL(
      ...
      const mappedProjects = retProjects.map((obj) =>
        obj.retItems.length ? obj.retItems[0] : []
      );
    ...
    selectedProjects.forEach((element) => {
      ...
      if (project) {
        const projectContact = prjContacts.find(
          (pc) => pc.ID === project.PrimaryPOC
        );
  ...
  async OnRowExpand(event) {
    ...
    event.data.invoices = [];
    let batchUrl = [];
    const url = this.spServices
      .getReadURL(
        this.constantService.listNames.InvoiceLineItems.name,
        this.fdConstantsService.fdComponent.invoiceLineItemProforma
      )
      .replace("{{ProformaLookup}}", event.data.Id);
    this.commonService.setBatchObject(
      batchUrl,
      url,
      null,
      this.constantService.Method.GET,
      this.constantService.listNames.InvoiceLineItems.type
    );
    ...
    if (Invoices.length > 0) {
      ...
      for (let i = 0; i < Invoices.length; i++) {
        ...
        const projcd = this.projectContactsData.find(
          (c) => c.ID === Invoices[i].MainPOC
        );
        const piInfo = this.projectInfoData.find(
          (c) => c.ProjectCode === Invoices[i].Title
        );
        event.data.invoices.push({
          ...
          ClientName:
            piInfo && piInfo.ClientLegalEntity ? piInfo.ClientLegalEntity : "",
          ScheduledDate: new Date(
            this.datePipe.transform(Invoices[i].ScheduledDate, "MMM dd, yyyy")
          ),
          ScheduledDateFormat: this.datePipe.transform(
            Invoices[i].ScheduledDate,
            "MMM dd, yyyy"
          ),
          ...
          PracticeArea:
            piInfo && piInfo.BusinessVertical ? piInfo.BusinessVertical : "",
          POName: poItem.Name,
  ...
  showProformaDialog() {
    ...
    ref.onClose.subscribe((proformaForm: any) => {
      if (proformaForm) {
        this.isPSInnerLoaderHidden = false;
        const batchUrl = this.fdDataShareServie.AddToProformaProcessData(
          proformaForm
        );
  ...
  sentToClient() {
    if (this.selectedRowData.length) {
      const filterData = this.selectedRowData.filter(
        (e) => e.Status !== "Created"
      );
      if (!filterData.length) {
        ...
        this.commonService.showToastrMessage(
          this.constantService.MessageType.info,
          "Please select only those Row Item whose Proforma is Created.",
          false
        );
      }
    } else {
      ...
      this.commonService.showToastrMessage(
        this.constantService.MessageType.info,
        "Please select one of Row Item & try again.",
        false
      );
    }
  }
