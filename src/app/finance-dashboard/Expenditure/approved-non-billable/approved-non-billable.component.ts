...
//   @ViewChild("anb", { static: false }) approvedNBTable: Table;
  ...
  createANBCols() {
    this.approvedNonBillableCols = [
      ...
      },
      {
        ...
        visibility: true,
        Type: "string",
        dbName: "ClientCurrency",
        options: [],
      },
      {
        field: "PayingEntity",
        header: "Paying Entity",
        visibility: true,
        Type: "string",
        dbName: "PayingEntity",
        options: [],
      ...
      },
      {
        ...
        visibility: true,
        Type: "string",
        dbName: "ApproverComments",
        options: [],
      },
      {
        field: "ModifiedDateFormat",
        header: "Approval / Billable Date",
        visibility: false,
      },
      {
        field: "ModifiedDate",
      ...
      {
        field: "ClientApprovalFileURL",
        header: "Client Approval File URL",
        visibility: false,
      },
      {
        field: "ApproverComments",
      ...
      {
        field: "ApproverFileUrl",
        header: "Approver File Url",
        visibility: false,
      },
  ...
  async getRequiredData() {
    ...
    if (
      ...
      speInfoObj = Object.assign(
        {},
        this.fdConstantsService.fdComponent.spendingInfoForNonBillable
      );
      ...
      speInfoObj = Object.assign(
        {},
        this.fdConstantsService.fdComponent.spendingInfoForNonBillableCS
      );
      speInfoObj.filter = speInfoObj.filter
        .replace("{{StartDate}}", this.DateRange.startDate)
        .replace("{{EndDate}}", this.DateRange.endDate)
  ...
  async formatData(data: any[]) {
    ...
    this.approvedNonBillableCols =
      this.approvedNonBillableRes && this.approvedNonBillableRes.length > 0
        ? this.commonService.MainfilterForTable(
            this.approvedNonBillableCols,
            this.approvedNonBillableRes
          )
        : this.approvedNonBillableCols.filter((c) => c.visibility === true);
  ...
  createColFieldValues(resArray: any[]) {
    this.anonBillableColArray.ProjectCode = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a: { ProjectCode: any }) => {
            const b = { label: a.ProjectCode, value: a.ProjectCode };
            return b;
          })
          .filter((ele: { label: any }) => ele.label)
      )
    );
    this.anonBillableColArray.Category = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a: { Category: any }) => {
    ...
    this.anonBillableColArray.ExpenseType = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a: { ExpenseType: any }) => {
    ...
    this.anonBillableColArray.ClientAmount = this.fdDataShareServie.customSort(
      clientAmount,
      1,
      "label"
    );
    this.anonBillableColArray.ClientCurrency = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a: { ClientCurrency: any }) => {
    ...
    this.anonBillableColArray.PaymentMode = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a: { PaymentMode: any }) => {
    ...
    this.anonBillableColArray.PayingEntity = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a: { PayingEntity: any }) => {
    ...
    this.anonBillableColArray.Status = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a: { Status: any }) => {
    ...
    this.anonBillableColArray.Number = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a: { Number: any }) => {
    ...
    this.anonBillableColArray.VendorName = this.commonService.sortData(
      this.uniqueArrayObj(
        resArray
          .map((a: { VendorName: any }) => {
    ...
    this.anonBillableColArray.PaymentDate = this.uniqueArrayObj(
      resArray
        .map((a: { DateSpend: any }) => {
    ...
    const modified = this.commonService.sortDateArray(
      this.uniqueArrayObj(
        resArray
          .map((a: { ModifiedDate: any }) => {
            const b = {
              label: this.datePipe.transform(a.ModifiedDate, "MMM dd, yyyy"),
    ...
    this.anonBillableColArray.ModifiedDate = modified
      .map((a: any) => {
        const b = {
      ...
      .filter((ele: { label: any }) => ele.label);
    this.anonBillableColArray.Created = this.uniqueArrayObj(
      resArray
        .map((a: { Created: any }) => {
  ...
  uniqueArrayObj(array: any) {
    let sts: any = "";
    return (sts = Array.from(
      new Set(array.map((s: { label: any }) => s.label))
    ).map((label1) => {
      return {
        label: label1,
  ...
  selectAllRows() {
    this.selectedAllRowsItem.length === 0
      ? (this.selectedRowItemData = [])
      : this.selectedRowItemData;
    if (
      this.selectedAllRowsItem.length === this.approvedNonBillableRes.length
    ) {
    ...
    for (let i = 0; i < this.selectedAllRowsItem.length; i++) {
      ...
      if (element.Status !== sts) {
        ...
        this.commonService.showToastrMessage(
          ...
          'Please select line item with status containing "Payment Pending" & try again.',
          false
        );
      }
    }
  ...
  openTableAtt(data: any, popUpData: any) {
    ...
    if (
      ...
      if (
        ...
        data.Status == "Approved Payment Pending"
      ) {
        this.items = [
          {
    ...
    this.items.push({
      label: "Details",
      command: (e: any) => this.openMenuContent(e, data),
    });
  }
      ...
      if (modal === "markAsPaymentModal") {
        ...
        if (!this.approvedSts) {
          this.commonService.showToastrMessage(
            ...
            'Please select line item with status containing "Payment Pending".',
            false
          );
          return false;
        }
  ...
  async MarkAsPayment(markAsPayment_form, type: string, fileUrl) {
    if (fileUrl) {
      ...
      this.selectedAllRowsItem.forEach((element: { Id: any }) => {
        ...
        spendingInfoObj.url = this.spServices.getItemURL(
          this.constantService.listNames.SpendingInfo.name,
          element.Id
        );
  ...
  sendApproveCanRejExpMail(expense, type: string) {
    ...
    const val1 = isCleData.hasOwnProperty("ClientLegalEntity")
      ? expense.ProjectCode + " (" + isCleData.ClientLegalEntity + ")"
      : expense.ProjectCode;
    ...
    mailContent = this.replaceContent(
      mailContent,
      "@@Val10@@",
      this.expenseForm.value.ApproverComments
        ? this.expenseForm.value.ApproverComments
        : this.expenseForm.value.ApproverComments
    );
    ...
    mailContent = this.replaceContent(
      mailContent,
      "@@Val13@@",
      author.hasOwnProperty("UserNamePG") ? author.UserNamePG.Title : "Member"
    );
    mailContent = this.replaceContent(
      mailContent,
      "@@Val14@@",
    ...
    if (type === "Reject Expense") {
      ...
      if (itApprovers.length) {
        arrayCC = arrayCC.concat(
          this.fdDataShareServie.getITMember(itApprovers)
        );
      }
    }
