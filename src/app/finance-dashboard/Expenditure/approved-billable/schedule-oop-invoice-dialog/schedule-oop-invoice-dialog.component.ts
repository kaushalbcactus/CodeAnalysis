...
export class ScheduleOopInvoiceDialogComponent implements OnInit {
  ...
  async poChange(event) {
    ...
    if (this.poItem) {
      ...
      if (!PoExist) {
        ...
        this.PoAdvanceInvoiceList.push({ Id: this.poItem.ID, advanceInvList: arrINV })
      }
      else {
      ...
      if (this.advanceInvArray.length > 0) {
        this.ScheduleInvoiceForm.get('InvoiceType').setValue(this.InvoiceTypeArray[0].value)
        this.UpdateValidator('existing');
      }
      else {
        this.ScheduleInvoiceForm.get('InvoiceType').setValue(this.InvoiceTypeArray[1].value)
        this.UpdateValidator('new');
      }

      let oopBalance = (this.poItem.AmountOOP ? this.poItem.AmountOOP - this.poItem.OOPLinked : 0 - (this.poItem.OOPLinked ? this.poItem.OOPLinked : 0));
      this.ScheduleInvoiceForm.get('OOPBalance').setValue(parseFloat(oopBalance.toFixed(2)));
    ...
    if (+(this.ScheduleInvoiceForm.get('OOPBalance').value) >= this.ScheduleInvoiceForm.getRawValue().Amount) {
      await this.getPfPfb();
    } else {
  ...
  async getPfPfb() {
    ...
    pfObj.url = this.spServices.getReadURL(this.constantsService.listNames.ProjectFinances.name,
      this.fdConstantsService.fdComponent.projectFinances);
    pfObj.url = pfObj.url.replace('{{ProjectCode}}', this.ScheduleInvoiceForm.getRawValue().ProjectCode);
    ...
    pfbObj.url = this.spServices.getReadURL(this.constantsService.listNames.ProjectFinanceBreakup.name,
      this.fdConstantsService.fdComponent.projectFinanceBreakupFromPO);
    pfbObj.url = pfbObj.url.replace('{{ProjectCode}}', this.ScheduleInvoiceForm.getRawValue().ProjectCode)
    ...
    pbbObj.url = this.spServices.getReadURL(this.constantsService.listNames.ProjectBudgetBreakup.name,
      this.fdConstantsService.fdComponent.projectBudgetBreakup);
    pbbObj.url = pbbObj.url.replace('{{ProjectCode}}', this.ScheduleInvoiceForm.getRawValue().ProjectCode);
    pbbObj.listName = this.constantsService.listNames.ProjectBudgetBreakup.name;
    pbbObj.type = 'GET';
    batchUrl.push(pbbObj);


    this.common.SetNewrelic('Finance-Dashboard', 'approved-billable', 'GetPFPFBPBB');
  ...
  getPInfoByPC() {
    const found = this.projectInfoData.find((x) => {
      if (x.ProjectCode === this.ScheduleInvoiceForm.getRawValue().ProjectCode) {
        return x;
      }
    });
    return found ? found : '';
  ...
  setInvData() {
    ...
    const oInv = this.arrAdvanceInvoices.find(e => e.ID === this.ScheduleInvoiceForm.get('InvoiceId').value);
    this.ScheduleInvoiceForm.get('InvoiceDate').setValue(oInv ? this.datePipe.transform(oInv.InvoiceDate, 'MMM, dd, yyyy') : '')
    this.ScheduleInvoiceForm.get('InvoiceAmount').setValue(oInv ? oInv.Amount : '');
    this.ScheduleInvoiceForm.get('BalanceAmount').setValue(oInv ? (oInv.Amount - oInv.TaggedAmount) : '');
    this.ScheduleInvoiceForm.get('TagAmount').setValue(this.ScheduleInvoiceForm.get('Amount').value);
    this.ScheduleInvoiceForm.get('TagAmount').setValidators([Validators.required, this.common.checkGTZeroNumberValidator(), Validators.max(oInv ? (oInv.Amount - oInv.TaggedAmount) : 0)]);
  ...
  UpdateValidator(value: string) {
    ...
    if (value === 'new') {
      this.ScheduleInvoiceForm.get('ScheduledDate').setValidators([Validators.required]);
      this.ScheduleInvoiceForm.get('POCName').setValidators([Validators.required]);
      this.ScheduleInvoiceForm.get('AddressType').setValidators([Validators.required]);
  ...
  SaveDetails() {
    if (this.ScheduleInvoiceForm.valid) {
      const data = {
        ...
        Invoice: this.arrAdvanceInvoices.length > 0 ? this.arrAdvanceInvoices.find(e => e.ID === this.ScheduleInvoiceForm.get('InvoiceId').value) : null
      }
      this.ref.close(data);
