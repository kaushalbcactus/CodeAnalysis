...
export class FDDataShareService {
  ...
  async getCurrentUserInfo() {
    ...
    return await this.spServices.getUserInfo(
  ...
  async getRequiredData(): Promise<any> {
    if (!this.requiredData.length) {
      ...
      this.globalObject.userInfo = await this.spServices.getUserInfo(
        this.globalObject.currentUser.userId
      );
      ...
      prjContactsObj.url = this.spServices.getReadURL(
        this.constantService.listNames.ProjectContacts.name,
        this.fdConstantsService.fdComponent.projectContacts
      );
      ...
      cleObj.url = this.spServices.getReadURL(
        this.constantService.listNames.ClientLegalEntity.name,
        this.fdConstantsService.fdComponent.clientLegalEntity
      );
      ...
      usStatesObj.url = this.spServices.getReadURL(
        this.constantService.listNames.UsState.name,
        this.fdConstantsService.fdComponent.usStates
      );
      ...
      currencyObj.url = this.spServices.getReadURL(
        this.constantService.listNames.Currency.name,
        this.fdConstantsService.fdComponent.currency
      );
      ...
      projectPOObj.url = this.spServices.getReadURL(
        this.constantService.listNames.PO.name,
        this.fdConstantsService.fdComponent.projectPO
      );
      ...
      resCatObj.url = this.spServices.getReadURL(
        this.constantService.listNames.ResourceCategorization.name,
        this.fdConstantsService.fdComponent.resourceCategorization
      );
      ...
      billingEntObj.url = this.spServices.getReadURL(
        this.constantService.listNames.BillingEntity.name,
        this.fdConstantsService.fdComponent.billingEntity
      );
      ...
      budgetRateObj.url = this.spServices.getReadURL(
        this.constantService.listNames.BudgetRateMaster.name,
        this.fdConstantsService.fdComponent.budgetRate
      );
      ...
      prjInfoObj.url = this.spServices.getReadURL(
        this.constantService.listNames.ProjectInformation.name,
        this.fdConstantsService.fdComponent.projectInfo
      );
      ...
      this.requiredData = arrResults = arrResults.length
        ? arrResults.map((a) => a.retItems)
        : [];
  ...
  async getClePO(type: string) {
    ...
    if (type === "hourly") {
      ...
      hourlyObj.url = this.spServices.getReadURL(
        this.constantService.listNames.ProjectInformation.name,
        this.fdConstantsService.fdComponent.projectInfo
      );
  ...
  async callProformaCreation(
    ...
    const oProformaObj: any = this.getProformaPDFObject(
      oProforma,
      cleData,
      projectContactsData,
      purchaseOrdersList,
    ...
    if (objReturn) {
      ...
      const oCLE = cleData.find(
        (e) => e.Title === oProformaObj.ClientLegalEntity
      );
      ...
      this.commonService.SetNewrelic(
        "Finance-Dashboard",
        "fd-shareData",
        "CallProformaCreation",
        "POST-BATCH"
      );
      const pdfService =
        "https://cactusspofinance.cactusglobal.com/pdfservice3/PDFService.svc/GeneratePDF";
  ...
  getProformaPDFObject(
    ...
    let val = "";
    const tax1 =
      oProformaObj.Template === "India"
        ? ((parseFloat(oProformaObj.Amount) * 18) / 100).toFixed(2)
        : oProformaObj.Template === "Japan"
        ? ((parseFloat(oProformaObj.Amount) * 10) / 100).toFixed(2)
        : "0";
    ...
    const currencyObj = this.bdtRate.find(
      (e) => e.Title === oProformaObj.Currency
    );
    ...
    return {
      ...
      invoice_no: oProformaObj.Title
        ? oProformaObj.Title
        : oProformaObj.ProformaNumber,
      invoice_date: this.datePipe.transform(
        oProformaObj.ProformaDate,
        "MMM d, y"
      ),
      ...
      clientcontact1:
        oProformaObj.Template === "US"
          ? oPOC.LName + ", " + oPOC.FName
          : oPOC.FName + " " + oPOC.LName, // 'Strachan , Gordon',
  ...
  async createInvoice(proformHtml, selectedRowItem, oInv, cleData) {
    let response = true;
    const proformaDate = this.datePipe.transform(
      selectedRowItem.ProformaDate,
      "MMM dd, yyyy"
    );
    const proformaDateSingle = this.datePipe.transform(
      selectedRowItem.ProformaDate,
      "MMM d, yyyy"
    );
    ...
    proformHtml = proformHtml.replace(new RegExp("PROFORMA", "g"), "INVOICE");
    proformHtml = proformHtml.replace(new RegExp("proforma", "g"), "invoice");
    proformHtml = proformHtml.replace(
      new RegExp(selectedRowItem.ProformaNumber, "g"),
      oInv.InvoiceNumber
    );
    proformHtml = proformHtml.replace(
      new RegExp(proformaDate, "g"),
      this.datePipe.transform(oInv.InvoiceDate, "MMM dd, yyyy")
    );
    proformHtml = proformHtml.replace(
      new RegExp(proformaDateSingle, "g"),
      this.datePipe.transform(oInv.InvoiceDate, "MMM dd, yyyy")
    );
  ...
  EditInvoiceDialogProcess(InvoiceType, data, editInvoice) {
    ...
    if (InvoiceType === "hourly") {
      ...
      iliData = {
        ...
        AddressType: editInvoice.value.AddressType,
  ...
  getCSDetails(res) {
    if (
      res.hasOwnProperty("Access") &&
      res.Access.hasOwnProperty("results") &&
      res.Access.results.length
    ) {
      const title = [];
  ...
  AddToProformaProcessData(proformaForm) {
    ...
    const prfData = {
      ...
      Template: proformaForm.value.Template,
      ...
      AddressType: proformaForm.value.AddressType,
    ...
    this.commonService.setBatchObject(
      batchUrl,
      this.spServices.getReadURL(this.constantService.listNames.Proforma.name),
      prfData,
    ...
    let cleData = {
      ...
      ProformaCounter: proformaForm.value.ClientLegalEntity.ProformaCounter
        ? proformaForm.value.ClientLegalEntity.ProformaCounter + 1
        : 1,
    };
    this.commonService.setBatchObject(
      batchUrl,
      this.spServices.getItemURL(
        this.constantService.listNames.ClientLegalEntity.name,
        +proformaForm.value.ClientLegalEntity.Id
      ),
      cleData,
      this.constantService.Method.PATCH,
      this.constantService.listNames.ClientLegalEntity.name
