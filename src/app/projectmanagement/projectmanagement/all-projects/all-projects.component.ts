...
export class AllProjectsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (
      this.pmObject.userRights.isMangers ||
      this.pmObject.userRights.isHaveProjectFullAccess
    ) {
      this.CSButtonEnable = true;
      this.FinanceButtonEnable = true;
    } else if (
      this.pmObject.userRights.isInvoiceTeam ||
      this.pmObject.resourceCatItems[0].RoleCH ===
        this.pmConstant.resourCatConstant.FINANCE
    ) {
      this.FinanceButtonEnable = true;
    } else if (
      this.pmObject.resourceCatItems[0].RoleCH ===
        this.pmConstant.resourCatConstant.CMLevel1 ||
      this.pmObject.resourceCatItems[0].RoleCH ===
        this.pmConstant.resourCatConstant.CMLevel2
    ) {
      this.CSButtonEnable = true;
    }
  ...
  async checkEarlyTaskCompleted() {
    const completedTaskFilter = Object.assign(
      {},
      this.pmConstant.QUERY.GET_EARLY_TASK_COMPLETED
    );
    const lastOneHour = this.commonService.ConvertTimeformat(
      24,
    ...
    completedTaskFilter.filter = completedTaskFilter.filter
      .replace("{{UserID}}", this.globalObject.currentUser.userId.toString())
    ...
    if (sResult && sResult.length) {
      ...
      for (const element of sResult) {
        ...
        this.commonService.showToastrMessage(
          ...
          "Early task " + element.Title + " has completed successfully.",
          true
        );
        remainingUserId = projectCSArray
          .filter((x) => x.ID !== this.globalObject.currentUser.userId)
          .map((x) => x.ID);
        ...
        const retResults = await this.spServices.updateItem(
          ...
          element.ID,
          earlyTask,
          this.constants.listNames.EarlyTaskCompleteNotifications.type
        );
      }
    }
  }
  ...
  async convertToExcelFile(data) {
    ...
    const AllBudgets = budgets
      .filter((c) => c.retItems[0] !== undefined)
      .map((c) => c.retItems[0]);
    console.log(AllBudgets);
    this.pmObject.allProjectsArray.forEach((project) => {
      const projBudget = AllBudgets.find(
        (c) => c.Title === project.ProjectCode
      );
      if (projBudget) {
        project.RevenueBudget = projBudget.RevenueBudget
          ? projBudget.RevenueBudget
          : 0;
  ...
  reloadAllProject() {
        ...
        items: [
          {
            ...
            command: (event) =>
              this.changeProjectStatus(
                ...
                this.pmConstant.ACTION.CONFIRM_PROJECT
              ),
          },
          {
            ...
            command: (event) =>
              this.changeProjectStatus(
                this.selectedProjectObj,
                this.pmConstant.ACTION.PROPOSE_CLOSURE
              ),
          },
          {
            label: "Audit Project",
            command: (event) =>
              this.changeProjectStatus(
                this.selectedProjectObj,
                this.pmConstant.ACTION.AUDIT_PROJECT
              ),
          },
          {
            label: "Close Project",
            command: (event) =>
              this.changeProjectStatus(
                this.selectedProjectObj,
                this.pmConstant.ACTION.CLOSE_PROJECT
              ),
          },
          {
            ...
            command: (event) =>
              this.changeProjectStatus(
                this.selectedProjectObj,
                this.pmConstant.ACTION.CANCEL_PROJECT
              ),
          },
          {
            label: "On Hold",
            command: (event) =>
              this.InvoiceLineItemsPopup(
                this.selectedProjectObj,
                this.constants.projectStatus.OnHold
              ),
          },
          {
            label: "Off Hold",
          ...
          {
            label: "Move to Finance Audit",
            command: (event) => this.revertFromClosed(this.selectedProjectObj),
          },
          {
            label: "Move to CS Audit",
            command: (event) =>
        ...
        items: [
          {
            ...
            command: (event) => this.viewProject(this.selectedProjectObj),
          },
          {
            label: "Edit Project",
            command: (event) => this.editProject(this.selectedProjectObj),
          },
          {
            label: "Edit Deliverable Type",
            command: (event) => this.editDeliverable(),
          },
          {
            label: "Edit Project Date",
            command: (event) => this.editProjectDate(),
          },
          {
            label: "Communications",
            command: (event) => this.communications(this.selectedProjectObj),
          },
          {
            label: "Timeline",
            command: (event) => this.projectTimeline(this.selectedProjectObj),
          },
          {
            label: "Go to Allocation",
            command: (event) =>
              this.goToAllocationPage(this.selectedProjectObj),
          },
          {
            label: "Project Scope",
            command: (event) => this.goToProjectScope(this.selectedProjectObj),
          },
          {
            label: "View CDs/PFs",
            command: (event) => this.viewCDPF(this.selectedProjectObj),
          },
          {
            label: "View PubSupport",
            command: (event) =>
              this.viewPubSupportJCDetails(this.selectedProjectObj),
          },
        ],
      },
      {
        ...
        items: [
          {
            ...
            command: (event) => this.manageFinances(this.selectedProjectObj),
          },
          {
            label: "Change SOW",
            command: (event) => this.moveSOW(this.selectedProjectObj),
          },
          {
            label: "Expense",
            command: (event) => this.showExpense(this.selectedProjectObj),
          },
          {
            label: "Budget Breakup",
            command: (event) =>
              this.showProjectBudgetBreakup(this.selectedProjectObj),
          },
        ],
      },
      {
        label: "View Details",
        command: (event) => this.sendOutput.next(this.selectedProjectObj),
      },
      {
        label: "Show History",
        command: (event) => this.showTimeline(this.selectedProjectObj),
      },
    ];
    this.getAllProjects();
    ...
    this.subscription = this.dataService
      .on("reload-project")
      .subscribe(() => this.callReloadProject());
  }
  /**
  ...
  async getAllProjects() {
    ...
    if (this.pmObject.allProjectItems.length) {
      ...
      const projectObj = this.pmObject.allProjectItems.filter(
        (c) => c.ProjectCode === this.params.ProjectCode
      );
      if (this.params.ActionStatus) {
        if (
          projectObj &&
          projectObj.length &&
          this.params.ActionStatus === this.pmConstant.ACTION.APPROVED
        ) {
          if (
            this.pmObject.userRights.isMangers ||
            projectObj[0].CMLevel2.ID === this.globalObject.currentUser.userId
          ) {
            if (
              projectObj[0].Status ===
              this.constants.projectStatus.AwaitingCancelApproval
            ) {
              await this.getGetIds(
                projectObj[0],
                this.pmConstant.ACTION.CANCEL_PROJECT
              );
              ...
              this.commonService.showToastrMessage(
                ...
                "Cancellation action on this project - " +
                  this.params.ProjectCode +
            ...
            this.commonService.showToastrMessage(
              this.constants.MessageType.error,
              "You are not authorized to Approve cancellation for this project - " +
                this.params.ProjectCode +
                " .",
              true
            );
        ...
        if (
          projectObj &&
          projectObj.length &&
          this.params.ActionStatus === this.pmConstant.ACTION.REJECTED
        ) {
          ...
          if (
            ...
            this.commonService.showToastrMessage(
              this.constants.MessageType.error,
              "You are not authorized to Reject cancellation for this project - " +
                this.params.ProjectCode +
                " .",
              true
            );
      ...
      if (this.params.ProjectBudgetStatus) {
        if (projectObj && projectObj.length) {
          if (
            ...
            await this.approveRejectBudgetReduction(
              this.params.ProjectBudgetStatus,
              projectObj[0]
            );
          } else {
            this.commonService.showToastrMessage(
              this.constants.MessageType.error,
              "You are not authorized to Approve / Reject budget reduction for this project - " +
                this.params.ProjectCode +
                " .",
              true
            );
    ...
    if (this.pmObject.allProjectItems && this.pmObject.allProjectItems.length) {
      ...
      for (const task of this.pmObject.allProjectItems) {
        ...
        projObj.modifiedStatus =
          task.Status === "Audit In Progress"
            ? "CS Audit"
        ...
        projObj.CreatedDateFormat = this.datePipe.transform(
          new Date(projObj.CreatedDate),
          "MMM dd yyyy hh:mm:ss aa"
        );
        projObj.ModifiedBy = task.Editor ? task.Editor.Title : "";
        projObj.ModifiedDate = task.Modified;
        projObj.ModifiedDateFormat = projObj.ModifiedDate
          ? this.datePipe.transform(
              new Date(projObj.ModifiedDate),
              "MMM dd yyyy hh:mm:ss aa"
            )
          : null;
        projObj.PrimaryPOC = task.PrimaryPOC;
        projObj.PrimaryPOCText = this.pmCommonService.extractNamefromPOC([
          projObj.PrimaryPOC,
        ]);
        projObj.POC = projObj.PrimaryPOCText[0];
        projObj.AdditionalPOC = task.POC ? task.POC.split(";#") : [];
        projObj.AdditionalPOCText = this.pmCommonService.extractNamefromPOC(
          task.POC ? task.POC.split(";#") : []
        );
        ...
        projObj.ConferenceJournal = task.ConferenceJournal
          ? task.ConferenceJournal
          : "";
        ...
        projObj.StandardBudgetHrs = task.StandardBudgetHrs
          ? task.StandardBudgetHrs
          : 0;
        projObj.CMLevel1ID =
          task.CMLevel1 && task.CMLevel1.results && task.CMLevel1.results.length
            ? task.CMLevel1.results
            : null;
        projObj.CMLevel1Title =
          projObj.CMLevel1ID && projObj.CMLevel1ID.length
            ? this.pmCommonService
                .extractNameFromId(projObj.CMLevel1ID)
                .join(";#")
            : "";
        projObj.CMLevel2ID = task.CMLevel2.ID;
        projObj.CMLevel2Title = task.CMLevel2.Title;
        projObj.DeliveryLevel1ID =
          task.DeliveryLevel1 &&
          task.DeliveryLevel1.results &&
          task.DeliveryLevel1.results.length
            ? task.DeliveryLevel1.results
            : null;
        projObj.DeliveryLevel1Title =
          projObj.DeliveryLevel1ID && projObj.DeliveryLevel1ID.length
            ? this.pmCommonService
                .extractNameFromId(projObj.DeliveryLevel1ID)
                .join(";#")
            : "";
        ...
        projObj.BusinessVertical = task.BusinessVertical
          ? task.BusinessVertical
          : "";
        projObj.BillingEntity = task.BillingEntity ? task.BillingEntity : "";
        projObj.SOWLink = task.SOWLink ? task.SOWLink : "";
        projObj.PubSupportStatus = task.PubSupportStatus
          ? task.PubSupportStatus
          : "";
        projObj.IsStandard = task.IsStandard ? task.IsStandard : "";
        projObj.StandardService = task.StandardService
          ? task.StandardService
          : "";
        ...
        projObj.AnnotationBinder = task.AnnotationBinder
          ? task.AnnotationBinder
          : "No";
        projObj.PrimaryResources = this.commonService.returnText(
          task.PrimaryResMembers.results
        );
        projObj.PrimaryResourcesId = this.commonService.getResourceId(
          task.PrimaryResMembers.results
        );
        projObj.LastSubmissionDate = task.LastSubmissionDate
          ? task.LastSubmissionDate
          : "";
        projObj.JournalSelectionDate = task.JournalSelectionDate
          ? task.JournalSelectionDate
          : "";
        projObj.JournalSelectionURL = task.JournalSelectionURL
          ? task.JournalSelectionURL
          : "";
        ...
        sowCodeTempArray.push({
          label: projObj.SOWCode,
          value: projObj.SOWCode,
        });
        projectCodeTempArray.push({
          label: projObj.ProjectCode,
          value: projObj.ProjectCode,
        });
        shortTitleTempArray.push({
          label: projObj.ShortTitle,
          value: projObj.ShortTitle,
        });
        clientLegalEntityTempArray.push({
          label: projObj.ClientLegalEntity,
          value: projObj.ClientLegalEntity,
        });
        // deliveryTypeTempArray.push({ label: projObj.DeliverableType, value: projObj.DeliverableType });

        // console.log(projObj);
        projectTypeTempArray.push({
          label: projObj.ProjectType,
          value: projObj.ProjectType,
        });
        statusTempArray.push({ label: projObj.Status, value: projObj.Status });

        TATempArray.push({ label: projObj.TA, value: projObj.TA });
        MoleculeTempArray.push({
          label: projObj.Molecule,
          value: projObj.Molecule,
        });
        POCTempArray.push({
    ...
    if (this.pmObject.allProjectsArray.length) {
      if (
        this.pmObject.allProjectsArray[0].Status ===
          this.constants.projectList.status.Cancelled ||
        this.pmObject.allProjectsArray[0].Status ===
          this.constants.projectList.status.Closed
      ) {
  ...
  async approveRejectBudgetReduction(selectedStatus, projectObj) {
    ...
    sowGet.url = this.spServices.getReadURL(
    ...
    if (result && result.length) {
      ...
      const pbb = existPBBArray.find(
        (e) =>
          e.Status ===
          this.constants.projectBudgetBreakupList.status.ApprovalPending
      );
      if (pbb) {
        ...
        if (selectedStatus === this.pmConstant.ACTION.APPROVED) {
          ...
          sowUpdate.url = this.spServices.getItemURL(
            this.constants.listNames.SOW.name,
            existSOW.ID
          );
          ...
          projectFinanceUpdate.url = this.spServices.getItemURL(
        ...
        projectBudgetBreakupUpdate.url = this.spServices.getItemURL(
          this.constants.listNames.ProjectBudgetBreakup.name,
          pbb.ID
        );
        ...
        const subjectVal =
          "Budget reduction is " + selectedStatus.toLowerCase();
        const mailSubject = projectObj.ProjectCode + ": " + subjectVal;
        const objEmailBody = [];
        objEmailBody.push({
          key: "@@ProjectCode@@",
          value: projectObj.ProjectCode,
        });
        objEmailBody.push({
          key: "@@ClientName@@",
          value: projectObj.ClientLegalEntity,
        });
        ...
        this.pmCommonService.getTemplate(
          this.constants.EMAIL_TEMPLATE_NAME.BUDGET_APPROVAL,
          objEmailBody,
          mailSubject,
          arrayTo,
          ccIDs
        ...
        this.commonService.showToastrMessage(
          this.constants.MessageType.error,
          "No un-apporved budget reduction request found for project - " +
            this.params.ProjectCode +
            " .",
          true
        );
  ...
  lazyLoadTask(event) {
    ...
    this.commonService.lazyLoadTask(
      event,
      allProjectArray,
      this.filterColumns,
      this.pmConstant.filterAction.ALL_PROJECTS
    );
  }
  /**
  ...
  storeRowData(rowData, menu) {
    ...
    if (route.indexOf("myDashboard") > -1) {
      ...
      menu.model[1].items[3].visible =
        this.selectedProjectObj.ProjectType ===
        this.pmConstant.PROJECT_TYPE.FTE.value
          ? true
          : false;
      menu.model[1].items[9].visible =
        this.selectedProjectObj.IsPubSupport === "Yes" ? true : false;
      menu.model[1].items[2].visible =
        this.selectedProjectObj.IsStandard === "Yes" ? false : true;
    ...
    ref.onClose.subscribe((obj) => {
      if (obj) {
        if (obj.status == this.constants.projectStatus.Unallocated) {
          this.changeProjectStatusOffHoldtoUnallocated(
            selectedProjectObj,
            obj.invoiceLineItems
          );
        } else {
          this.changeProjectStatusOnHold(
            selectedProjectObj,
            obj.invoiceLineItems
          );
        }
  ...
  async changeProjectStatus(selectedProjectObj, projectAction) {
    ...
    if (result && result.length) {
      const scheduleItems = result.find(
        (c) => c.listName === this.constants.listNames.Schedules.name
      )
        ? result.find(
            (c) => c.listName === this.constants.listNames.Schedules.name
          ).retItems
      ...
      switch (projectAction) {
        ...
        case this.pmConstant.ACTION.PROPOSE_CLOSURE:
          const pbbItems = result.find(
            (c) =>
              c.listName === this.constants.listNames.ProjectBudgetBreakup.name
          )
            ? result.find(
                (c) =>
                  c.listName ===
          ...
          const InvoiceLineItems = result.find(
          ...
          if (InvoiceLineItems) {
            if (InvoiceLineItems.find((c) => c.Status === "Scheduled")) {
              this.commonService.showToastrMessage(
                this.constants.MessageType.error,
                selectedProjectObj.ProjectCode + " line item is not Confirmed.",
                true
              );
              this.loaderView.nativeElement.classList.remove("show");
              this.spannerView.nativeElement.classList.remove("show");
          ...
          const ExpenseLineItems = result.find(
        ...
        case this.pmConstant.ACTION.CANCEL_PROJECT:
          ...
          if (scheduleItems.length) {
            const SpentHours = scheduleItems
              .filter((c) => parseFloat(c.TimeSpent))
            ...
            if (SpentHours > 0) {
              this.commonService.showToastrMessage(
                this.constants.MessageType.error,
                "Cancellation not allowed as there is spent hours on the project.",
                true
              );
              return;
            }
  ...
  loadReasonDropDown() {
    this.cancelReasonArray = [
      {
        label: this.pmConstant.PROJECT_CANCELLED_REASON
          .NOT_CONVINCED_OF_EXPERIANCE_OR_QUALITY,
        value: this.pmConstant.PROJECT_CANCELLED_REASON
          .NOT_CONVINCED_OF_EXPERIANCE_OR_QUALITY,
      },
      {
      ...
      {
        label: this.pmConstant.PROJECT_CANCELLED_REASON
          .CLIENT_NOT_MOVING_FORWARD_WITH_PROJECT,
        value: this.pmConstant.PROJECT_CANCELLED_REASON
          .CLIENT_NOT_MOVING_FORWARD_WITH_PROJECT,
      },
      {
  ...
  async saveCancelProject() {
    if (this.selectedReasonType && this.selectedReason) {
      ...
      if (
        this.selectedProjectObj.Status !==
        this.constants.projectStatus.InDiscussion
      ) {
      ...
      if (
        ...
        this.sendApprovalEmailToManager(
          this.selectedProjectObj,
          this.selectedReason
        );
      }
  ...
  async sendApprovalEmailToManager(selectedProjectObj, reason) {
    const projectFinanceObj =
      this.toUpdateIds[1] &&
      this.toUpdateIds[1].retItems &&
      this.toUpdateIds[1].retItems.length
    ...
    objEmailBody.push({
      key: "@@Val1@@",
      value: selectedProjectObj.ProjectCode,
    });
    objEmailBody.push({
      key: "@@Val2@@",
      value: selectedProjectObj.ClientLegalEntity,
    });
    objEmailBody.push({
      key: "@@SiteName@@",
      value: this.globalObject.sharePointPageObject.webAbsoluteUrl,
    });
    objEmailBody.push({
      key: "@@Val3@@",
      value: this.pmObject.currLoginInfo.Title,
    });
    objEmailBody.push({
      key: "@@Val4@@",
      value: selectedProjectObj.ProposedStartDate,
    });
    ...
    objEmailBody.push({
      key: "@@ProjectStatus@@",
      value: selectedProjectObj.Status,
    });
    let tempArray = [];
    let arrayTo = [];
    const cm1IdArray = [];
    ...
    tempArray = tempArray.concat(
      cm1IdArray,
      this.selectedProjectObj.CMLevel2ID
    );
    arrayTo = this.pmCommonService.getEmailId(tempArray);
    ...
    arrayTo = arrayTo.filter(this.onlyUnique);
    this.pmCommonService.getTemplate(
      this.constants.EMAIL_TEMPLATE_NAME.CANCEL,
      objEmailBody,
      mailSubject,
      arrayTo,
    ...
    this.commonService.showToastrMessage(
      this.constants.MessageType.success,
      "Email has been sent for approval to concerned person.",
      true
    );
    setTimeout(() => {
      this.pmObject.allProjectItems = [];
  ...
  async changeProjectStatusCancelled(selectedProjectObj) {
    const projectBudgetBreakUPObjs =
      this.toUpdateIds[0] &&
      this.toUpdateIds[0].retItems &&
      this.toUpdateIds[0].retItems.length
        ? this.toUpdateIds[0].retItems
        : [];
    const projectFinanceObj =
      this.toUpdateIds[1] &&
      ...
      this.toUpdateIds[1].retItems.length
        ? this.toUpdateIds[1].retItems[0]
        : [];
    const projectFinanceBreakObjs =
      this.toUpdateIds[2] &&
      this.toUpdateIds[2].retItems &&
      this.toUpdateIds[2].retItems.length
        ? this.toUpdateIds[2].retItems
        : [];
    const sowObj =
      this.toUpdateIds[3] &&
      this.toUpdateIds[3].retItems &&
      this.toUpdateIds[3].retItems.length
        ? this.toUpdateIds[3].retItems[0]
        : [];
    const POObjsArray =
      this.toUpdateIds[4] &&
      this.toUpdateIds[4].retItems &&
      this.toUpdateIds[4].retItems.length
        ? this.toUpdateIds[4].retItems
        : [];
    const scheduleListObjs =
      this.toUpdateIds[5] &&
      this.toUpdateIds[5].retItems &&
      this.toUpdateIds[5].retItems.length
        ? this.toUpdateIds[5].retItems
        : [];
    const invoiceLineItemsObjs =
      this.toUpdateIds[6] &&
      this.toUpdateIds[6].retItems &&
      this.toUpdateIds[6].retItems.length
        ? this.toUpdateIds[6].retItems
        : [];
    let batchURL = [];
    const options = {
    ...
    piUpdate.type = "PATCH";
    piUpdate.url = this.spServices.getItemURL(
      this.constants.listNames.ProjectInformation.name,
    ...
    invoiceLineItemsObjs.forEach((element) => {
      ...
      invoiceUpdate.url = this.spServices.getItemURL(
    ...
    projectFinanceBreakObjs.forEach((element) => {
      ...
      projectFinanceBreakUpdate.url = this.spServices.getItemURL(
        this.constants.listNames.ProjectFinanceBreakup.name,
        element.ID
      );
    ...
    projectBudgetBreakUPObjs.forEach((element) => {
      if (
        selectedProjectObj.Status === this.constants.projectStatus.InDiscussion
      ) {
        ...
        projectBudgetUpdate.url = this.spServices.getItemURL(
          this.constants.listNames.ProjectBudgetBreakup.name,
          element.ID
        );
      ...
      if (
        ...
        element.OriginalBudget = element.OriginalBudget
          ? element.OriginalBudget
          : 0;
        ...
        projectBudgetBreakCreate.url = this.spServices.getReadURL(
          this.constants.listNames.ProjectBudgetBreakup.name,
          null
        );
    ...
    if (
      selectedProjectObj.ProjectType ===
        this.pmConstant.PROJECT_TYPE.DELIVERABLE.value ||
      ...
      sowObj.ScheduledRevenue = sowObj.ScheduledRevenue
      ...
      projectFinanceObj.InvoicesScheduled = projectFinanceObj.InvoicesScheduled
        ? projectFinanceObj.InvoicesScheduled
        : 0;
      projectFinanceObj.ScheduledRevenue = projectFinanceObj.ScheduledRevenue
        ? projectFinanceObj.ScheduledRevenue
        : 0;
      projectFinanceObj.Budget = projectFinanceObj.Budget
        ? projectFinanceObj.Budget
        : 0;
      projectFinanceObj.RevenueBudget = projectFinanceObj.RevenueBudget
      ...
      const sowData = {
        ...
        TotalScheduled:
          sowObj.TotalScheduled - projectFinanceObj.InvoicesScheduled,
        ScheduledRevenue:
          sowObj.ScheduledRevenue - projectFinanceObj.ScheduledRevenue,
      ...
      projectFinanceBreakObjs.forEach((element) => {
        ...
        if (poItem) {
          ...
          poItem.RevenueLinked = poItem.RevenueLinked
            ? poItem.RevenueLinked
            : 0;
          poItem.TotalScheduled = poItem.TotalScheduled
            ? poItem.TotalScheduled
            : 0;
          poItem.ScheduledRevenue = poItem.ScheduledRevenue
            ? poItem.ScheduledRevenue
            : 0;
          ...
          const poData = {
            ...
            ScheduledRevenue:
              poItem.ScheduledRevenue - element.ScheduledRevenue,
          };
          const poUpdate = Object.assign({}, options);
          poUpdate.data = poData;
          poUpdate.listName = this.constants.listNames.PO.name;
          poUpdate.type = "PATCH";
          poUpdate.url = this.spServices.getItemURL(
            this.constants.listNames.PO.name,
            poItem.ID
          );
    ...
    for (const element of scheduleListObjs) {
      if (batchURL.length < 100) {
        ...
        scUpdate.type = "PATCH";
        scUpdate.url = this.spServices.getItemURL(
    ...
    setTimeout(() => {
      if (this.router.url === "/projectMgmt/allProjects") {
        ...
        this.router.url.includes("/projectMgmt/allProjects?ProjectCode")
      ) {
        window.history.pushState(
          {},
          "Title",
  ...
  async changeProjectStatusClose(selectedProjectObj) {
    ...
    picloseUpdate.url = this.spServices.getItemURL(
      this.constants.listNames.ProjectInformation.name,
      selectedProjectObj.ID
    );
    ...
    this.commonService.showToastrMessage(
      this.constants.MessageType.success,
      "Project - " + selectedProjectObj.ProjectCode + " closed Successfully.",
      true
    );
    setTimeout(() => {
      if (this.router.url === "/projectMgmt/allProjects") {
  ...
  async getStandardTemplateBasedOnDeliverable(selectedProjectObj) {
    ...
    const standardTemplateFilter = Object.assign(
      {},
      this.pmConstant.QUERY.standardTemplateOptions
    );
    standardTemplateFilter.filter = standardTemplateFilter.filter
      .replace(/{{service}}/gi, selectedProjectObj.StandardService)
      .replace(/{{clientLegalEntity}}/gi, selectedProjectObj.ClientLegalEntity);
    standardTemplate = await this.spServices.readItems(
      this.constants.listNames.StandardTemplates.name,
      standardTemplateFilter
    );
  ...
  async changeProjectStatusUnallocated(selectedProjectObj) {
    ...
    if (
      ...
      selectedProjectObj.ProjectType ===
        this.pmConstant.PROJECT_TYPE.HOURLY.value &&
      selectedProjectObj.IsStandard == "Yes"
    ) {
      let standardTemplate = await this.getStandardTemplateBasedOnDeliverable(
        selectedProjectObj
      );
      if (
        standardTemplate.filter(
          (s) =>
            s.IsActiveCH == "Yes" &&
            s.AutoUpdate == true &&
            s.StandardService.Title == selectedProjectObj.StandardService
        )
      ) {
        const firstMilestone = selectedProjectObj.Milestones.split(";#");
        piUdateData.Milestone = firstMilestone[0];
        piUdateData.Status = this.constants.projectStatus.InProgress;
        await this.changeStatusStandardProject(
          selectedProjectObj,
          firstMilestone[0]
        );
      }
    ...
    projectBudgetBreakUPIds.forEach((element) => {
      ...
      prjBudgetBreakupUpdate.url = this.spServices.getItemURL(
        this.constants.listNames.ProjectBudgetBreakup.name,
        element.ID
      );
    ...
    this.commonService.showToastrMessage(
      this.constants.MessageType.success,
      "Project - " + selectedProjectObj.ProjectCode + " Updated Successfully.",
      true
    );
    setTimeout(() => {
      if (this.router.url === "/projectMgmt/allProjects") {
  ...
  async changeMilestoneStatusFTE(selectedProjectObj) {
    const scheduleFilter = Object.assign(
      {},
      this.pmConstant.QUERY.GET_SCHEDULE_LIST_ITEM_BY_PROJECT_CODE
    );
    scheduleFilter.filter = scheduleFilter.filter.replace(
      /{{projectCode}}/gi,
      selectedProjectObj.ProjectCode
    );
    this.commonService.SetNewrelic(
    ...
    if (sResult && sResult.length > 0) {
      const filterResult = sResult.filter(
        (a) => a.Title.indexOf(selectedProjectObj.monthName) > -1
      );
      const batchURL = [];
      const options = {
        data: null,
      ...
      filterResult.forEach((element) => {
        if (
          element.Task !== this.pmConstant.task.BLOCKING &&
          ...
          scheduleStatusUpdate.url = this.spServices.getItemURL(
            this.constants.listNames.Schedules.name,
            element.ID
          );
    ...
    if (sResult && sResult.length > 0) {
      ...
      sResult.forEach((element) => {
        if (element.Title == milestone) {
          ...
          scheduleMilestoneStatusUpdate.url = this.spServices.getItemURL(
            this.constants.listNames.Schedules.name,
            element.ID
          );
          ...
          const scheduleStatusUpdate = Object.assign({}, options);
          const statusNotStartedSchedule = Object.assign(
            {},
            statusNotStartedScheduleList
          );
          statusNotStartedSchedule.ActiveCA =
            element.ContentTypeCH === "Slot" ? "Yes" : "No";
      ...
      if (batchURL.length) {
        ...
        this.commonService.showToastrMessage(
          this.constants.MessageType.success,
          milestone + " Milestone is Auto Confirmed.",
          true
        );
      }
    }
  ...
  async changeProjectStatusAuditInProgress(selectedProjectObj, scheduleItems) {
    const projectFinanceID =
      this.toUpdateIds[1] &&
      this.toUpdateIds[1].retItems &&
      this.toUpdateIds[1].retItems.length
        ? this.toUpdateIds[1].retItems[0].ID
        : -1;
    const batchURL = [];
    const options = {
      data: null,
    ...
    objMilestone.filter = objMilestone.filter.replace(
      /{{projectCode}}/gi,
      selectedProjectObj.ProjectCode
    );
    this.commonService.SetNewrelic(
    ...
    if (allTasks.length > 0) {
      ...
      const milestones = allTasks.filter(
        (c) =>
          ...
          (c.Status === "Not Confirmed" || c.Status === "In Progress")
      );
      ...
      const allMilestoneTasks = allTasks.filter(
        (c) =>
          c.ContentTypeCH !== this.constants.CONTENT_TYPE.MILESTONE &&
          ...
          if (milestone.SubMilestones) {
            ...
            if (SubMilestones) {
              SubMilestones.forEach((element) => {
                const status =
                  element.split(":")[2] === "Not Started" ||
                  element.split(":")[2] === "In Progress"
                    ? "Auto Closed"
                    : element.split(":")[2];
                if (status !== "Not Confirmed") {
                  SubMilestonesObj.push(
                    element.split(":")[0] +
                      ":" +
                      element.split(":")[1] +
                      ":" +
              ...
              modifiedSubMilestones =
                SubMilestonesObj.length > 0
                  ? SubMilestonesObj.join(";#")
                  : null;
            }
          }
          const data = {
            Status:
              milestone.Status === "Not Confirmed" ? "Deleted" : "Auto Closed",
    ...
    piInfoUpdate.url = this.spServices.getItemURL(
      this.constants.listNames.ProjectInformation.name,
      this.selectedProjectObj.ID
    );
    ...
    const hourSpent = await this.getTotalHours(
    ...
    if (
      ...
      this.sendEmailBasedOnStatus(
        this.constants.projectStatus.SentToAMForApproval,
        selectedProjectObj
      );
    } else {
      this.sendEmailBasedOnStatus(
        this.constants.projectStatus.AuditInProgress,
        selectedProjectObj
      );
    }
    this.commonService.showToastrMessage(
  ...
  async changeProjectStatusOnHold(selectedProjectObj, invoiceLineItems) {
    ...
    const filterTasks = tasks.filter(
      (e) =>
        e.Task !== "Select one" &&
        e.Task !== "Client Review" &&
        e.Status !== this.constants.STATUS.DELETED &&
    ...
    filterTasks.forEach((element) => {
      ...
      if (element.Task == "Client Review") {
        ...
        const scheduleStatusUpdate = Object.assign({}, options);
        const scInProgressUpdateDataNew = Object.assign(
          {},
          scInProgressUpdateData
        );
        scInProgressUpdateDataNew.ExpectedTime = element.TimeSpent;
        scInProgressUpdateDataNew.DueDateDT =
          new Date(element.DueDateDT) < new Date()
            ? new Date(element.DueDateDT)
            : new Date();
  ...
  async changeProjectStatusOffHoldtoUnallocated(
    selectedProjectObj,
    invoiceLineItems
  ) {
    const batchURL = [];
  ...
  async revertFromClosed(selectedProjectObj) {
    this.commonService
      .confirmMessageDialog(
        "Change Status of Project -" + selectedProjectObj.ProjectCode + "",
        "Are you sure you want to change the Status of Project - " +
          selectedProjectObj.ProjectCode +
          "" +
          " from '" +
    ...
    this.commonService
      ...
      .then(async (Confirmation) => {
        if (Confirmation === "Yes") {
          ...
          if (batchURL.length) {
            this.groupITInfo = await this.spServices.getGroupInfo(
              "Invoice_Team"
            );
            this.commonService.SetNewrelic(
              "projectManagment",
            ...
            await this.sendNotificationMail(
              this.constants.EMAIL_TEMPLATE_NAME.NOTIFY_PM,
              "Revert to CS Audit",
              selectedProjectObj,
              status,
              true
  ...
  async getGetIds(selectedProjectObj, projectAction) {
    ...
    const projectBBFilter = Object.assign(
      {},
      this.pmConstant.QUERY.PROJECT_BUDGET_BREAKUP_CANCELLED_BY_PROJECTCODE
    );
    projectBBFilter.filter = projectBBFilter.filter.replace(
      /{{projectCode}}/gi,
      selectedProjectObj.ProjectCode
    );
    projectBBGet.url = this.spServices.getReadURL(
      this.constants.listNames.ProjectBudgetBreakup.name,
      projectBBFilter
    );
    ...
    if (projectAction === this.pmConstant.ACTION.CANCEL_PROJECT) {
      ...
      const projectFinanaceBreakupFilter = Object.assign(
        {},
        this.pmConstant.QUERY.PROJECT_FINANCE_BREAKUP_CANCELLED_BY_PROJECTCODE
      );
      projectFinanaceBreakupFilter.filter = projectFinanaceBreakupFilter.filter.replace(
        /{{projectCode}}/gi,
        selectedProjectObj.ProjectCode
      );
      projectFinanceBreakupGet.url = this.spServices.getReadURL(
        this.constants.listNames.ProjectFinanceBreakup.name,
      ...
      poFilter.filter = poFilter.filter.replace(
        /{{clientLegalEntity}}/gi,
        selectedProjectObj.ClientLegalEntity
      );
      poGet.url = this.spServices.getReadURL(
        this.constants.listNames.PO.name,
        poFilter
      );
      poGet.type = "GET";
      ...
      const scheduleFilter = Object.assign(
        {},
        this.pmConstant.QUERY.SCHEDULE_LIST_BY_PROJECTCODE
      );
      scheduleFilter.filter = scheduleFilter.filter.replace(
        /{{projectCode}}/gi,
        selectedProjectObj.ProjectCode
      );
      scheduleGet.url = this.spServices.getReadURL(
        this.constants.listNames.Schedules.name,
        scheduleFilter
      );
  ...
  GetprojectFinanceBatch(projObj, options, batchURL) {
    ...
    const projectFinanceFilter = Object.assign(
      {},
      this.pmConstant.FINANCE_QUERY.PROJECT_FINANCE_BY_PROJECTCODE
    );
    projectFinanceFilter.filter = projectFinanceFilter.filter.replace(
      /{{projectCode}}/gi,
      projObj.ProjectCode
    );
    projectFinanceGet.url = this.spServices.getReadURL(
      this.constants.listNames.ProjectFinances.name,
  ...
  async getTotalHours(projectCode, isScheduleListStatusUpdated, scheduleItems) {
    ...
    if (scheduleItems.length === 0) {
      const scheduleFilter = Object.assign(
        {},
        this.pmConstant.QUERY.GET_TIMESPENT
      );
      scheduleFilter.filter = scheduleFilter.filter.replace(
        /{{projectCode}}/gi,
        projectCode
    ...
    if (sResult && sResult.length > 0) {
      ...
      sResult.forEach((task) => {
        if (
          !(
            task.Task === this.pmConstant.task.FOLLOW_UP &&
            task.Task === this.pmConstant.task.SEND_TO_CLIENT
          )
        ) {
          totalSpentTime += task.TimeSpent
            ? this.timeToMins(task.TimeSpent)
            : 0;
        }
        if (isScheduleListStatusUpdated) {
          ...
          if (task.SubMilestones && task.Milestone === "Select one") {
            newSubmilestones = "";
            const replaceString =
              task.SubMilestones.indexOf(";#") > -1
                ? task.SubMilestones.split(";#")
                : [];
            replaceString.forEach((element) => {
              if (element) {
                const subMilestoneTaskEle = element.split(":");
                if (
                  subMilestoneTaskEle.length &&
                  subMilestoneTaskEle[2] === this.constants.STATUS.IN_PROGRESS
                ) {
  ...
  async viewProject(selectedProjectObj) {
    ...
    this.pmObject.addProject.ProjectAttributes.ID = proj.hasOwnProperty("ID")
      ? proj.ID
      : 0;
    ...
    this.pmObject.addProject.ProjectAttributes.PointOfContact1 =
      proj.PrimaryPOC;
    this.pmObject.addProject.ProjectAttributes.PointOfContact1Text =
      proj.PrimaryPOCText[0];
    this.pmObject.addProject.ProjectAttributes.PointOfContact2 =
      proj.AdditionalPOC && proj.AdditionalPOC.length ? proj.AdditionalPOC : [];
    const pocIds = this.pmObject.addProject.ProjectAttributes.PointOfContact2.map(
      (x) => parseInt(x, 10)
    );
    this.pmObject.addProject.ProjectAttributes.PointOfContact2Text = this.pmCommonService
      .extractNamefromPOC(pocIds)
      .join(", ");
    ...
    this.pmObject.addProject.ProjectAttributes.ReferenceCount =
      proj.ReferenceCount;
    this.pmObject.addProject.ProjectAttributes.SlideCount = proj.SlideCount;
    this.pmObject.addProject.ProjectAttributes.AnnotationBinder =
      proj.AnnotationBinder;
    ...
    if (this.pmObject.addProject.Timeline.Standard.IsStandard) {
      ...
      this.pmObject.addProject.Timeline.Standard.ProposedStartDate =
        proj.ProposedStartDate;
      this.pmObject.addProject.Timeline.Standard.ProposedEndDate =
        proj.ProposedEndDate;
    }
    if (this.pmObject.addProject.Timeline.NonStandard.IsStandard) {
      this.pmObject.addProject.Timeline.NonStandard.Service =
        proj.StandardService;
      ...
      this.pmObject.addProject.Timeline.NonStandard.SubDeliverable =
        proj.SubDeliverable;
      this.pmObject.addProject.Timeline.NonStandard.ProposedStartDate =
        proj.ProposedStartDate;
      this.pmObject.addProject.Timeline.NonStandard.ProposedEndDate =
        proj.ProposedEndDate;
    }
    let cm1Array = [];
    let delivery1Array = [];
    ...
    this.pmObject.addProject.ProjectAttributes.ActiveCM1Text = this.pmCommonService
      .extractNameFromId(cm1Array)
      .join(", ");
    this.pmObject.addProject.ProjectAttributes.ActiveCM2 = proj.CMLevel2ID
      ? proj.CMLevel2ID
      : 0;
    this.pmObject.addProject.ProjectAttributes.ActiveCM2Text = proj.CMLevel2Title
      ? proj.CMLevel2Title
      : "";
    this.pmObject.addProject.ProjectAttributes.ActiveDelivery1 = delivery1Array;
    this.pmObject.addProject.ProjectAttributes.ActiveDelivery1Text = this.pmCommonService
      .extractNameFromId(delivery1Array)
      .join(", ");
    this.pmObject.addProject.ProjectAttributes.ActiveDelivery2 = proj.DeliveryLevel2ID
      ? proj.DeliveryLevel2ID
      : 0;
    this.pmObject.addProject.ProjectAttributes.ActiveDelivery2Text = proj.DeliveryLevel2Title
      ? proj.DeliveryLevel2Title
      : "";
  ...
  async getProjectFinanceData(proj) {
    ...
    if (sResult && sResult.length > 0) {
      ...
      if (this.pmObject.addProject.FinanceManagement.ProjectSOW) {
        if (
          this.pmObject.addProject.FinanceManagement.ProjectSOW.indexOf(
            this.globalObject.sharePointPageObject.webRelativeUrl
          ) === -1
        ) {
          const client = this.pmObject.oProjectCreation.oProjectInfo.clientLegalEntities.find(
            (e) => e.Title === proj.ClientLegalEntity
          );
  ...
  goToAllocationPage(task) {
    window.open(
      this.globalObject.url + "/taskAllocation?ProjectCode=" + task.ProjectCode,
      "_blank"
    );
  }
  ...
  async sendNotificationMail(
    ...
    if (status == this.constants.projectStatus.OnHold) {
      ...
      arrayTo = arrayTo.concat(
        delivery1Id,
        selectedProjectObj.DeliveryLevel2ID,
        primaryRes
      );
      ...
      if (revertStatusCsAudit) {
        objEmailBody.push({
          key: `'Unallocated'`,
          value: "Moved to CS Audit from Finance Audit",
        });
      ...
      if (status === this.constants.projectStatus.SentToAMForApproval) {
        ...
        objEmailBody.push({
          key: "@@Val3@@",
          value: selectedProjectObj.DeliverableType,
        });
        objEmailBody.push({
          key: "@@Val4@@",
          value: selectedProjectObj.ClientLegalEntity,
        ...
        objEmailBody.push({
          key: "@@Val5@@",
          value: selectedProjectObj.PrimaryPOCText,
        });
        objEmailBody.push({
          key: "@@Val8@@",
          value: selectedProjectObj.PrimaryPOCText,
      ...
      if (status !== this.constants.projectStatus.Unallocated) {
        ...
        if (status === this.constants.projectStatus.SentToAMForApproval) {
          ...
          objEmailBody.push({
            key: "@@Val7@@",
            value: selectedProjectObj.StandardBudgetHrs,
          });
        } else {
    ...
    switch (status) {
      case this.constants.projectStatus.AuditInProgress:
        this.sendNotificationMail(
          this.constants.EMAIL_TEMPLATE_NAME.AUDIT_PROJECT,
      ...
      case this.constants.projectStatus.PendingClosure:
        this.sendNotificationMail(
          this.constants.EMAIL_TEMPLATE_NAME.CLOSE_PROJECT,
          "Audit completed kindly close the project",
          selectedProjectObj,
          status
        );
        break;
      ...
      case this.constants.projectStatus.SentToAMForApproval:
        this.sendNotificationMail(
          this.constants.EMAIL_TEMPLATE_NAME.APPROVED_PROJECT,
          "Approve project for billing",
          selectedProjectObj,
          status
        );
        break;
      case this.constants.projectStatus.OnHold:
        this.sendNotificationMail(
          this.constants.EMAIL_TEMPLATE_NAME.ON_HOLD,
          "On Hold",
          selectedProjectObj,
          status
        );
  ...
  async updateUsedHrs() {
    ...
    if (this.selectedProjectObj.ProjectCode) {
      totalHours = await this.getTotalHours(
        this.selectedProjectObj.ProjectCode,
        false,
        []
      );
  ...
  async editProject(projObj) {
    ...
    const ref = this.dialogService.open(ProjectAttributesComponent, {
      header:
        "Edit Project - " + projObj.ProjectCode + "(" + projObj.Title + ")",
      width: "90vw",
      data: {
        projectObj: projObj,
      },
  ...
  async moveSOW(projObj) {
    ...
    if (sowArray && sowArray.length) {
      sowArray.forEach((element) => {
        this.sowDropDownArray.push({
          label: element.SOWCode,
          value: element.SOWCode,
        });
      });
    }
    this.pmObject.isMainLoaderHidden = true;
  ...
  async showExpense(projObj) {
    ...
    result.expanse.forEach((element) => {
      this.projectExpenses.push({
        ...
        ClientAmount: element.ClientAmount
          ? parseFloat(parseFloat(element.ClientAmount).toFixed(2))
          : 0,
        ClientCurrency: element.ClientCurrency,
        ModifiedBy: element.Editor ? element.Editor.Title : "",
        Modified: new Date(
          this.datePipe.transform(element.Modified, "MMM dd, yyyy")
        ),
        ModifiedDateFormat: this.datePipe.transform(
          element.Modified,
          "MMM dd, yyyy, hh:mm a"
        ),
    ...
    this.expenseColArray.VendorName = this.commonService.sortData(
      this.uniqueArrayObj(
        this.projectExpenses
          .map((a) => {
    ...
    this.expenseColArray.RequestType = this.commonService.sortData(
      this.uniqueArrayObj(
        this.projectExpenses
          .map((a) => {
    ...
    this.expenseColArray.Status = this.commonService.sortData(
      this.uniqueArrayObj(
        this.projectExpenses
          .map((a) => {
    ...
    this.expenseColArray.Category = this.commonService.sortData(
      this.uniqueArrayObj(
        this.projectExpenses
          .map((a) => {
    ...
    this.expenseColArray.ExpenseType = this.commonService.sortData(
      this.uniqueArrayObj(
        this.projectExpenses
          .map((a) => {
    ...
    this.expenseColArray.ClientAmount = this.pmCommonService.customSort(
      ClientAmount,
      1,
      "label"
    );
    this.expenseColArray.ClientCurrency = this.commonService.sortData(
      this.uniqueArrayObj(
        this.projectExpenses
          .map((a) => {
    ...
    this.expenseColArray.ModifiedBy = this.uniqueArrayObj(
      this.projectExpenses
        .map((a) => {
          const b = { label: a.ModifiedBy, value: a.ModifiedBy };
          return b;
        })
        .filter((ele) => ele.label)
    );
    this.expenseColArray.ModifiedDate = this.uniqueArrayObj(
      this.projectExpenses
        .map((a) => {
          const b = {
            label: this.datePipe.transform(a.Modified, "MMM dd, yyyy"),
            value: a.Modified,
          };
          return b;
        })
    ...
    this.projectTitle = projObj.ShortTitle
      ? projObj.ProjectCode + " (" + projObj.ShortTitle + ") "
      : projObj.ProjectCode;
  ...
  async getExpanseByProjectCode(projectCode) {
    ...
    VendorFreeLancerGet.url = this.spServices.getReadURL(
      "" + this.constants.listNames.VendorFreelancer.name + ""
    );
  ...
  async performSOWMove() {
    ...
    const isValid = await this.validateSOWBudgetForProjectMovement(
      this.newSelectedSOW,
      projObject
    );
    if (isValid) {
      if (isValid === "InvoiceNotScheduled") {
        this.commonService.showToastrMessage(
          this.constants.MessageType.error,
          "Project Movement to selected SOW " +
            this.newSelectedSOW +
            " is not possible due to confirmed invoice",
          false
        );
        this.pmObject.isMainLoaderHidden = true;
        return;
      ...
      const newSOWObj = allSOWArray.filter(
        (x) => x.SOWCode === this.newSelectedSOW
      );
      const oldSOWObj = allSOWArray.filter(
        (x) => x.SOWCode === projObject.SOWCode
      );
      if (newSOWObj && newSOWObj.length && oldSOWObj && oldSOWObj.length) {
        const newSOW = newSOWObj[0];
        const oldSOW = oldSOWObj[0];
        const newSOWTotalBudget = isNaN(newSOW.TotalLinked)
          ? projObject.Budget
          : newSOW.TotalLinked + projObject.Budget;
        const newSOWRevenueBudget = isNaN(newSOW.RevenueLinked)
          ? projObject.RevenueBudget
          : newSOW.RevenueLinked + projObject.RevenueBudget;
        const newSOWOOPBudget = isNaN(newSOW.OOPLinked)
          ? projObject.OOPBudget
          : newSOW.OOPLinked + projObject.OOPBudget;
        const newSOWTaxBudget = isNaN(newSOW.TaxLinked)
          ? projObject.TaxBudget
          : newSOW.TaxLinked + projObject.TaxBudget;
        const newSOWTotalScheduled = isNaN(newSOW.TotalScheduled)
          ? projObject.InvoicesScheduled
          : newSOW.TotalScheduled + projObject.InvoicesScheduled;
        const newSOWScheduledRevenue = isNaN(newSOW.ScheduledRevenue)
          ? projObject.ScheduledRevenue
          : newSOW.ScheduledRevenue + projObject.ScheduledRevenue;
        const newSOWScheduledOOP = isNaN(newSOW.ScheduledOOP)
          ? projObject.ScheduledOOP
          : newSOW.ScheduledOOP + projObject.ScheduledOOP;
        const newSOWData = this.getSOWData(
          newSOWTotalBudget,
          newSOWRevenueBudget,
          newSOWOOPBudget,
          newSOWTaxBudget,
          newSOWTotalScheduled,
          newSOWScheduledRevenue,
          newSOWScheduledOOP
        );
        const newSOWUpdate = Object.assign({}, options);
        newSOWUpdate.url = this.spServices.getItemURL(
          this.constants.listNames.SOW.name,
          newSOW.ID
        );
        ...
        const oldSOWRevenueBudget =
          oldSOW.RevenueLinked - projObject.RevenueBudget;
        const oldSOWOOPBudget = oldSOW.OOPLinked - projObject.OOPBudget;
        const oldSOWTaxBudget = oldSOW.TaxLinked - projObject.TaxBudget;
        const oldSOWTotalScheduled =
          oldSOW.TotalScheduled - projObject.InvoicesScheduled;
        const oldSOWScheduledRevenue =
          oldSOW.ScheduledRevenue - projObject.ScheduledRevenue;
        const oldSOWScheduledOOP =
          oldSOW.ScheduledOOP - projObject.ScheduledOOP;
        const oldSOWData = this.getSOWData(
          oldSOWTotalBudget,
          oldSOWRevenueBudget,
          oldSOWOOPBudget,
          oldSOWTaxBudget,
          oldSOWTotalScheduled,
          oldSOWScheduledRevenue,
          oldSOWScheduledOOP
        );
        const oldSOWUpdate = Object.assign({}, options);
        oldSOWUpdate.url = this.spServices.getItemURL(
          this.constants.listNames.SOW.name,
          oldSOW.ID
        );
        ...
        projectInfoUpdate.url = this.spServices.getItemURL(
          this.constants.listNames.ProjectInformation.name,
          projObject.ID
        );
        ...
        this.commonService.showToastrMessage(
          this.constants.MessageType.success,
          "Project move to under new SOW Code - " + this.newSelectedSOW + "",
          true
        );
        setTimeout(() => {
          if (this.router.url === "/projectMgmt/allProjects") {
        ...
        if (newSOWObj.length === 0) {
          this.commonService.showToastrMessage(
            this.constants.MessageType.error,
            "Can't move Project - " +
              projObject.ProjectCode +
              " under new SOW Code as new sow - " +
              this.newSelectedSOW +
              " is closed / cancelled",
            true
          );
      ...
      this.commonService.showToastrMessage(
        ...
        "Project Movement to selected SOW " +
          this.newSelectedSOW +
          " is not possible due to insufficient amount",
        false
      );
    }
  }
  ...
  async validateSOWBudgetForProjectMovement(newSOWCode, projObj) {
    ...
    if (sResult && sResult.length && sowObj && sowObj.length) {
      ...
      if (
        ...
        projObj.InvoicesScheduled = fm.InvoicesScheduled
          ? fm.InvoicesScheduled
          : 0;
        projObj.ScheduledRevenue = fm.ScheduledRevenue
          ? fm.ScheduledRevenue
          : 0;
        projObj.ScheduledOOP = fm.ScheduledOOP ? fm.ScheduledOOP : 0;
        const sowBalanceTotalBudget =
          sowItem.TotalBudget - (sowItem.TotalLinked ? sowItem.TotalLinked : 0);
        const sowBalanceRevenueBudget =
          sowItem.NetBudget -
          (sowItem.RevenueLinked ? sowItem.RevenueLinked : 0);

        if (
          sowBalanceTotalBudget >= projObj.Budget &&
  ...
  async getProjectByCode() {
    ...
    if (
      ...
      projectInfoFilter = Object.assign(
        {},
        this.pmConstant.PM_QUERY.PROJECT_INFORMATION_BY_PROJECTCODE_ALL
      );
    } else {
      projectInfoFilter = Object.assign(
        {},
        this.pmConstant.PM_QUERY.PROJECT_INFORMATION_BY_PROJECTCODE
      );
      projectInfoFilter.filter = projectInfoFilter.filter.replace(
        "{{UserID}}",
        this.globalObject.currentUser.userId.toString()
      );
    }
  ...
  showAllCSFinanceAudit(title) {
    const ProjectList =
      title === "Finance"
        ? this.pmObject.allProjectsArray.filter(
            (c) => c.Status === "Pending Closure"
          )
        : this.pmObject.allProjectsArray.filter(
            (c) => c.Status === "Audit In Progress"
          );

    const ProjectFinanceIDs =
      this.toUpdateIds[1] &&
      this.toUpdateIds[1].retItems &&
      this.toUpdateIds[1].retItems.length
        ? this.toUpdateIds[1].retItems[0].ID
  ...
  async editDeliverable() {
    ...
    if (result.length) {
      result.forEach((element) => {
        this.deliverableTypeOptions.push({
          label: element.Title,
          value: element.Title,
        });
      });
  ...
  projectDateChange(type) {
    if (type == "start") {
      ...
      this.isMonthIncluded = this.selectedProjectObj.Milestones.split(
        ";#"
      ).filter(
        (e) =>
          e == this.commonService.getMonthName(new Date(this.proposedStartDate))
      );
    } else if (type === "end") {
      // let startMonth = this.commonService.getMonthName(this.proposedStartDate);
      // let endMonth = this.commonService.getMonthName(this.proposedEndDate);
      this.isMonthIncluded = this.selectedProjectObj.Milestones.split(
        ...
        (e) =>
          e == this.commonService.getMonthName(new Date(this.proposedEndDate))
      );
    }
  }
  ...
  async getResources() {
    let resources = [];
    let resouceData = Object.assign(
      {},
      this.pmConstant.FINANCE_QUERY.GET_RESOUCEBYID
    );
    resouceData.filter = resouceData.filter.replace(
      /{{Id}}/gi,
      this.selectedProjectObj.PrimaryResourcesId[0].Id
    );
  ...
  updateDetails(startDate, endDate, type) {
    ...
    if (type == "update") {
      ...
      taskData["ExpectedTime"] = resourceObj
        ? "" + businessDay * resourceObj.MaxHrs
        : "0";
  ...
  async deleteMilestones(milestones, monthsObj, batchURL) {
    let deleteMonthArr = milestones.filter(
      (e) => !monthsObj.find((m) => m.monthName == e)
    );
  ...
  async saveProjectDate() {
    ...
    const getProjectFolder = Object.assign(
      {},
      this.pmConstant.ProjectInformation_Get_ProjectFolder_Options
    );
    getProjectFolder.filter = getProjectFolder.filter.replace(
      /{{projectCode}}/gi,
      this.selectedProjectObj.ProjectCode
    );
    const projectFolderResults = await this.spServices.readItems(
      this.constants.listNames.ProjectInformation.name,
      getProjectFolder
    );
    const projectFolder =
      projectFolderResults && projectFolderResults.length
        ? projectFolderResults[0].ProjectFolder
        : "";
    this.allMilestones = this.selectedProjectObj.Milestones.split(";#");
    let months = this.pmCommonService.getMonths(
    ...
    if (
      this.commonService.getMonthName(new Date(this.proposedStartDate)) ===
        this.commonService.getMonthName(new Date(this.proposedEndDate)) &&
      this.isMonthIncluded.length
    ) {
      let data = this.updateDetails(
        this.proposedStartDate,
        this.proposedEndDate,
        "update"
      );
      let monthsObj = this.pmCommonService.getMonths(
        new Date(this.proposedStartDate),
        new Date(this.proposedEndDate)
      );
      let milestones = this.selectedProjectObj.Milestones.split(";#");
      for (let i = 0; i < this.scheduledTasks.length; i++) {
        ...
        for (const index in monthArr) {
          if (monthArr.hasOwnProperty(index)) {
            ...
            for (let i = 0; i < this.scheduledTasks.length; i++) {
              let task = this.scheduledTasks[i];
              if (
                task.ContentTypeCH == "Task" &&
                task.Milestone == element.monthName
              ) {
                await this.commonService.setBatchObject(
                  batchURL,
                  this.spServices.getItemURL(
      ...
      if (monthsObj.filter((e) => !milestones.includes(e.monthName)).length) {
        ...
        let monthArr = monthsObj.filter(
          (e) => !milestones.includes(e.monthName)
        );
        const monthName = monthArr.map((a) => a.monthName);
        this.allMilestones = [...this.allMilestones, ...monthName];
        let batchResults = await this.pmCommonService.createFTEMilestones(
          "",
          projectFolder,
          monthArr,
          this.resources[0],
    ...
    (this.proposedStartDate = null), (this.proposedEndDate = null);
    setTimeout(() => {
      if (this.router.url === "/projectMgmt/allProjects") {
        this.pmObject.allProjectItems = [];
        this.reloadAllProject();
