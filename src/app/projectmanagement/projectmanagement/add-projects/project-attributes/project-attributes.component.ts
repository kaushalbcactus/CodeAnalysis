...
export class ProjectAttributesComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    setTimeout(async () => {
      ...
      if (this.projObj) {
        ...
        const sow = this.pmObject.allSOWItems.filter(
          (objt) => objt.SOWCode === this.pmObject.addProject.SOWSelect.SOWCode
        );
        if (sow && sow.length) {
          this.addProjectAttributesForm
            .get("billedBy")
            .setValue(this.pmObject.addProject.FinanceManagement.BilledBy);
          this.addProjectAttributesForm.get("billedBy").disable();
          ...
          const sowObj = {
            ...
            AdditionalPOC: sow[0].AdditionalPOC
              ? sow[0].AdditionalPOC.split(";#").map((x) => parseInt(x, 10))
              : [],
          };
          await this.setFieldProperties(
            this.pmObject.addProject.ProjectAttributes,
            sowObj,
            true
          );
          ...
          await this.pmCommonService.assignValueToParameter(
            this.pmObject.addProject.ProjectAttributes
          );
        }
      }
    ...
    this.addProjectAttributesForm
      ...
      .valueChanges.subscribe((val) => {
        this.constant.RuleParamterArray.find(
          (c) => c.parameterName === "BusinessVertical"
        ).value = val;
  ...
  async setFieldProperties(projObj, sowObj, isCreate) {
    ...
    if (isCreate && sowObj) {
      this.addProjectAttributesForm
        .get("clientLeagalEntity")
        .setValue(sowObj.ClientLegalEntity);
      this.addProjectAttributesForm
        .get("billingEntity")
        .setValue(sowObj.BillingEntity);
      ...
      projObj.ClientLegalEntity = this.addProjectAttributesForm.get(
        "clientLeagalEntity"
      ).value;
    } else {
      this.addProjectAttributesForm
        .get("clientLeagalEntity")
        .setValue(projObj.ClientLegalEntity);
      this.addProjectAttributesForm
        .get("billingEntity")
        .setValue(projObj.BillingEntity);
    }
    if (projObj.SubDivision) {
      this.addProjectAttributesForm
        .get("subDivision")
        .setValue(projObj.SubDivision);
    }
    if (projObj.BilledBy) {
    ...
    if (projObj.PracticeArea) {
      this.addProjectAttributesForm
        .get("practiceArea")
        .setValue(projObj.PracticeArea);

      if (
        projObj.PracticeArea.toLowerCase() === "medinfo" ||
        projObj.PracticeArea.toLowerCase() === "medcom" ||
        projObj.PracticeArea.toLowerCase() === "medcomm"
      ) {
    ...
    if (projObj.ProjectCode) {
      this.addProjectAttributesForm
        .get("projectCode")
        .setValue(projObj.ProjectCode);
    }
    this.addProjectAttributesForm
      .get("projectStatus")
      .setValue(this.constant.projectStatus.InDiscussion);
    if (projObj.PointOfContact1) {
      this.addProjectAttributesForm
        .get("poc")
        .setValue(projObj.PointOfContact1);
    }
    if (projObj.PointOfContact2.length) {
      this.addProjectAttributesForm
        .get("poc2")
        .setValue(projObj.PointOfContact2);
    }
    if (projObj.Molecule) {
    ...
    if (projObj.TherapeuticArea) {
      this.addProjectAttributesForm
        .get("therapeuticArea")
        .setValue(projObj.TherapeuticArea);
    }
    if (projObj.Indication) {
      this.addProjectAttributesForm
        .get("indication")
        .setValue(projObj.Indication);
    }
    if (projObj.PUBSupportRequired) {
      this.addProjectAttributesForm
        .get("pubSupport")
        .setValue(projObj.PUBSupportRequired);
    }
    ...
    if (projObj.ProjectTitle) {
      this.addProjectAttributesForm
        .get("projectTitle")
        .setValue(projObj.ProjectTitle);
    }
    if (projObj.AlternateShortTitle) {
      this.addProjectAttributesForm
        .get("shortTitle")
        .setValue(projObj.AlternateShortTitle);
    }
    if (projObj.EndUseofDeliverable) {
      this.addProjectAttributesForm
        .get("endUseDeliverable")
        .setValue(projObj.EndUseofDeliverable);
    }
    if (projObj.SOWBoxLink) {
      this.addProjectAttributesForm
        .get("sowBoxLink")
        .setValue(projObj.SOWBoxLink);
    }
    if (projObj.ConferenceJournal) {
      this.addProjectAttributesForm
        .get("conference")
        .setValue(projObj.ConferenceJournal);
    }
    if (projObj.Authors) {
    ...
    if (projObj.PageCount) {
      this.addProjectAttributesForm
        .get("PageCount")
        .setValue(projObj.PageCount);
    }
    if (projObj.ReferenceCount) {
      this.addProjectAttributesForm
        .get("ReferenceCount")
        .setValue(projObj.ReferenceCount);
    }
    if (projObj.SlideCount) {
      this.addProjectAttributesForm
        .get("SlideCount")
        .setValue(projObj.SlideCount);
    }
    if (projObj.AnnotationBinder) {
      this.addProjectAttributesForm
        .get("AnnotationBinder")
        .setValue(projObj.AnnotationBinder);
    }
  ...
  async setDropDownValue(clientLeagalEntity) {
    ...
    this.commonService.SetNewrelic(
      "projectManagment",
      "addproj-projectAttributes",
      "GetClientSubdivision",
      "GET"
    );
    const result = await this.spServices.readItems(
      this.constant.listNames.ClientSubdivision.name,
      queryOptions
    );
    if (result && result.length) {
      result.forEach((element) => {
        this.subDivisionArray.push({
          label: element.Title,
          value: element.Title,
        });
      });
    ...
    this.pmObject.oProjectCreation.Resources.cmLevel1.forEach((element) => {
      this.pmObject.OwnerAccess.cmLevel1.push({
        label: element.Title,
        value: element.ID,
      });
      this.pmObject.OwnerAccess.cmLevel2.push({
        label: element.Title,
        value: element.ID,
      });
    });
    ...
    this.pmObject.oProjectCreation.Resources.deliveryLevel1.forEach(
      (element) => {
        this.pmObject.OwnerAccess.deliveryLevel1.push({
          label: element.Title,
          value: element.ID,
        });
        this.pmObject.OwnerAccess.deliveryLevel2.push({
          label: element.Title,
          value: element.ID,
        });
      }
    );
    this.pmObject.oProjectCreation.Resources.deliveryLevel2.forEach(
      (element) => {
        this.pmObject.OwnerAccess.deliveryLevel1.push({
          label: element.Title,
          value: element.ID,
    ...
    this.pmObject.OwnerAccess.cmLevel1.sort((a, b) =>
      a.label > b.label ? 1 : -1
    );
    this.pmObject.OwnerAccess.cmLevel2.sort((a, b) =>
      a.label > b.label ? 1 : -1
    );
    this.pmObject.OwnerAccess.deliveryLevel1.sort((a, b) =>
      a.label > b.label ? 1 : -1
    );
    this.pmObject.OwnerAccess.deliveryLevel2.sort((a, b) =>
      a.label > b.label ? 1 : -1
    );
  }
  /**
   ...
   */
  goToTimeline(data) {
    if (this.subDivisionArray.length) {
      const subDivision = this.addProjectAttributesForm.get("subDivision")
        .value;
      if (!subDivision) {
        this.commonService.showToastrMessage(
          this.constant.MessageType.error,
          "Please select subdivision",
          false
        );
        return false;
      }
    ...
    if (this.enableCountFields) {
      if (
        this.addProjectAttributesForm.value.ReferenceCount === null ||
        this.addProjectAttributesForm.value.ReferenceCount < 0
      ) {
        this.CountError = true;
        this.errorType = "Reference";
      } else if (
        this.addProjectAttributesForm.value.SlideCount === null ||
        this.addProjectAttributesForm.value.SlideCount < 0
      ) {
        this.CountError = true;
        this.errorType = "Slide";
      } else if (
        this.addProjectAttributesForm.value.PageCount === null ||
        this.addProjectAttributesForm.value.PageCount < 0
      ) {
        this.CountError = true;
        this.errorType = "Page";
      } else {
  ...
  EnableDisableCountFields() {
    this.CountError = false;
    const practiceValue = this.addProjectAttributesForm
      .get("practiceArea")
      .value.toLowerCase();
    if (
      practiceValue === "medinfo" ||
      practiceValue === "medcomm" ||
      practiceValue === "medcom"
    ) {
      this.enableCountFields = true;
    } else {
      this.enableCountFields = false;
   ...
   */
  setFormFieldValue() {
    this.pmObject.addProject.ProjectAttributes.ClientLegalEntity = this.addProjectAttributesForm.get(
      "clientLeagalEntity"
    ).value;
    this.pmObject.addProject.ProjectAttributes.SubDivision = this.addProjectAttributesForm.get(
      "subDivision"
    ).value;
    this.pmObject.addProject.ProjectAttributes.BillingEntity = this.addProjectAttributesForm.get(
      "billingEntity"
    ).value;
    this.pmObject.addProject.ProjectAttributes.BilledBy = this.addProjectAttributesForm.get(
      "billedBy"
    ).value;
    this.pmObject.addProject.ProjectAttributes.PracticeArea = this.addProjectAttributesForm.get(
      "practiceArea"
    ).value;
    this.pmObject.addProject.ProjectAttributes.Priority = this.addProjectAttributesForm.get(
      "priority"
    ).value;
    this.pmObject.addProject.ProjectAttributes.ProjectCode = this.addProjectAttributesForm.get(
      "projectCode"
    ).value;
    this.pmObject.addProject.ProjectAttributes.ProjectStatus = this.addProjectAttributesForm.get(
      "projectStatus"
    ).value;
    this.pmObject.addProject.ProjectAttributes.PointOfContact1 = this.addProjectAttributesForm.get(
      "poc"
    ).value;
    this.pmObject.addProject.ProjectAttributes.PointOfContact2 = this.addProjectAttributesForm.get(
      "poc2"
    ).value;
    this.pmObject.addProject.ProjectAttributes.Molecule = this.addProjectAttributesForm.get(
      "molecule"
    ).value;
    this.pmObject.addProject.ProjectAttributes.TherapeuticArea = this.addProjectAttributesForm.get(
      "therapeuticArea"
    ).value;
    this.pmObject.addProject.ProjectAttributes.Indication = this.addProjectAttributesForm.get(
      "indication"
    ).value;
    this.pmObject.addProject.ProjectAttributes.PUBSupportRequired = this.addProjectAttributesForm.get(
      "pubSupport"
    ).value;
    this.pmObject.addProject.ProjectAttributes.PUBSupportStatus = this.addProjectAttributesForm.get(
      "pubSupportStatus"
    ).value;
    this.pmObject.addProject.ProjectAttributes.ActiveCM1 = this.addProjectAttributesForm.get(
      "selectedActiveCM1"
    ).value;
    this.pmObject.addProject.ProjectAttributes.ActiveCM2 = this.addProjectAttributesForm.get(
      "selectedActiveCM2"
    ).value;
    this.pmObject.addProject.ProjectAttributes.ActiveDelivery1 = this.addProjectAttributesForm.get(
      "selectedActiveAD1"
    ).value;
    this.pmObject.addProject.ProjectAttributes.ActiveDelivery2 = this.addProjectAttributesForm.get(
      "selectedActiveAD2"
    ).value;
    this.pmObject.addProject.ProjectAttributes.ProjectTitle = this.addProjectAttributesForm.get(
      "projectTitle"
    ).value;
    this.pmObject.addProject.ProjectAttributes.AlternateShortTitle = this.addProjectAttributesForm.get(
      "shortTitle"
    ).value;
    this.pmObject.addProject.ProjectAttributes.EndUseofDeliverable = this.addProjectAttributesForm.get(
      "endUseDeliverable"
    ).value;
    this.pmObject.addProject.ProjectAttributes.SOWBoxLink = this.addProjectAttributesForm.get(
      "sowBoxLink"
    ).value;
    this.pmObject.addProject.ProjectAttributes.ConferenceJournal = this.addProjectAttributesForm.get(
      "conference"
    ).value;
    this.pmObject.addProject.ProjectAttributes.Authors = this.addProjectAttributesForm.get(
      "authors"
    ).value;
    this.pmObject.addProject.ProjectAttributes.Comments = this.addProjectAttributesForm.get(
      "comments"
    ).value;
    ...
    if (
      ...
      this.pmObject.addProject.ProjectAttributes.AnnotationBinder =
        this.addProjectAttributesForm.get("AnnotationBinder").value === null
          ? false
          : this.addProjectAttributesForm.get("AnnotationBinder").value;
      this.pmObject.addProject.ProjectAttributes.SlideCount = this.addProjectAttributesForm.get(
        "SlideCount"
      ).value
        ? this.addProjectAttributesForm.get("SlideCount").value
        : 0;
      this.pmObject.addProject.ProjectAttributes.PageCount = this.addProjectAttributesForm.get(
        "PageCount"
      ).value
        ? this.addProjectAttributesForm.get("PageCount").value
        : 0;
      this.pmObject.addProject.ProjectAttributes.ReferenceCount = this.addProjectAttributesForm.get(
        "ReferenceCount"
      ).value
        ? this.addProjectAttributesForm.get("ReferenceCount").value
        : 0;
    } else {
  ...
  onBilledByChanged() {
    if (
      this.addProjectAttributesForm.get("billedBy").value ===
  ...
  async editProject(projObj) {
    if (projObj.ActualStartDate) {
      ...
      const allowedDate = this.commonService.CalculateminstartDateValue(
        new Date(),
        3
      );
      if ((
    ...
    this.pmObject.addProject.ProjectAttributes.PUBSupportRequired =
      projObj.IsPubSupport === "Yes" ? true : false;
    this.pmObject.addProject.ProjectAttributes.PUBSupportStatus =
      projObj.PubSupportStatus;
    const poc2Array = [];
    if (
    ...
    this.pmObject.addProject.ProjectAttributes.ActiveCM1 =
      cm1IdArray && cm1IdArray.length ? cm1IdArray : [];
    ...
    this.pmObject.addProject.ProjectAttributes.ActiveDelivery1 =
      deliveryIdArray && deliveryIdArray.length ? deliveryIdArray : [];
    this.pmObject.addProject.ProjectAttributes.ActiveDelivery2 =
      projObj.DeliveryLevel2ID;
    ...
    this.pmObject.addProject.ProjectAttributes.ReferenceCount =
      projObj.ReferenceCount;
    this.pmObject.addProject.ProjectAttributes.PageCount = projObj.PageCount;
    this.pmObject.addProject.ProjectAttributes.AnnotationBinder = projObj.AnnotationBinder
      ? projObj.AnnotationBinder === "Yes"
        ? true
        : false
      : false;
  ...
  async saveEditProject() {
    if (this.selectedFile && this.selectedFile.size === 0) {
      this.commonService.showToastrMessage(
        this.constant.MessageType.error,
        this.constant.Messages.ZeroKbFile.replace(
          "{{fileName}}",
  ...
  async SaveProject() {
    if (this.addProjectAttributesForm.valid) {
      if (
        ...
        const Message = !this.pmObject.OwnerAccess.selectedCMOwner
          ? "CM"
          : "Delivery";
        this.commonService.showToastrMessage(
          this.constant.MessageType.warn,
          Message +
            " Owner is required.Project rule not defined for " +
            Message +
            ". Please define at-least one " +
            Message +
            " rule to update project.",
          false
        );
        return false;
      }
      ...
      if (this.selectedFile) {
        ...
        SelectedFile.push(
          new Object({ name: this.selectedFile.name, file: this.selectedFile })
        );
        this.commonService.SetNewrelic(
          "projectManagment",
          "addproj-projectAttributes",
          "UpdateProjectInformationfileupload",
          "POST-BATCH"
        );
          ...
          .then((uploadedfile) => {
            if (
              ...
              if (
                ...
                uploadedfile[0].hasOwnProperty("Name") &&
  ...
  async continueSaveProject() {
    const projectInfo = this.pmCommonService.getProjectData(
  ...
  async getFinanceData(projectObj) {
    ...
    const invoiceLineItemsFilter = Object.assign(
      {},
      this.pmConstant.FINANCE_QUERY.INVOICE_LINE_ITEMS_BY_PROJECTCODE
    );
    invoiceLineItemsFilter.filter = invoiceLineItemsFilter.filter.replace(
      /{{projectCode}}/gi,
      projectObj.ProjectCode
    );
    invoiceLineItemsGet.url = this.spServices.getReadURL(
    ...
    const spendingInfoFilter = Object.assign(
      {},
      this.pmConstant.FINANCE_QUERY.SPENDING_INFOByActive
    );
    spendingInfoFilter.filter = spendingInfoFilter.filter.replace(
      /{{Title}}/gi,
      projectObj.ProjectCode
    );
    spendingInfoGet.url = this.spServices.getReadURL(
  ...
  async updateFinanceData() {
    const batchUrl = [];
    let financeData = await this.getFinanceData(
      this.pmObject.addProject.ProjectAttributes
    );
  ...
  async addMoleculeToList() {
    ...
    if (this.addMolecule.valid) {
      ...
      val = val.trim();
      const molecule = this.pmObject.oProjectCreation.oProjectInfo.molecule.find(
        (e) => e.label === val
      );
      if (molecule) {
        this.commonService.showToastrMessage(
      ...
      moleculeItemCreate.url = this.spServices.getReadURL(
  ...
  async GetRulesOnChange() {
    if (this.showEditSave) {
      ...
      this.addProjectAttributesForm
        .get("selectedActiveCM1")
        .setValue(this.pmObject.OwnerAccess.selectedCMAccess);
      this.addProjectAttributesForm
        .get("selectedActiveAD1")
        .setValue(this.pmObject.OwnerAccess.selectedDeliveryAccess);
      this.addProjectAttributesForm
        .get("selectedActiveCM2")
        .setValue(this.pmObject.OwnerAccess.selectedCMOwner);
      this.addProjectAttributesForm
        .get("selectedActiveAD2")
