...
export class AddAccessService {
  ...
  async processData(batchURL, filterData) {
    ...
    filterData.CLE =
      filterData.CLE && filterData.CLE.length > 0
        ? filterData.CLE.map((e) => e["value"])
            .map((e, i, final) => final.indexOf(e) === i && i)
            .filter((e) => filterData.CLE[e])
            .map((e) => filterData.CLE[e])
        : [];
  ...
  async saveRules(filterData, type) {
    ...
    for (let i = 0; i < Rules.length; i++) {
      if (
        ...
        const exRule = filterData.DBRULES.find((c) => c.ID === Rules[i].ID)
          ? filterData.DBRULES.find((c) => c.ID === Rules[i].ID)
          : Rules[i];
    ...
    if (batchResultsArray && batchResultsArray.length > 0) {
      addedRuleArray = batchResultsArray.filter(
        (c) => c.listName === "RuleStoreCT"
      )
        ? batchResultsArray
            .filter((c) => c.listName === "RuleStoreCT")
            .map((c) => c.retItems)
        : [];
    }
    ...
    for (const index in dbItemList) {
      if (dbItemList.hasOwnProperty(index)) {
        let projectInfo = projectData.find(
          (e) => e.ProjectCode == dbItemList[index].Title
        );
        if (projectInfo) {
  ...
  async updateAllDetails(filterData, type, addedRuleArray, EditedRuleArray) {
    ...
    switch (type) {
      ...
      case this.constants.RulesType.CD:
        dbItemList =
          batchResults.find(
            (c) =>
              c.listName === this.constants.listNames.QualityComplaints.name
          ) &&
          batchResults.find(
            (c) =>
              c.listName === this.constants.listNames.QualityComplaints.name
      ...
      case this.constants.RulesType.PF:
        dbItemList =
          batchResults.find(
            (c) =>
              c.listName === this.constants.listNames.PositiveFeedbacks.name
          ) &&
          batchResults.find(
            (c) =>
              c.listName === this.constants.listNames.PositiveFeedbacks.name
    ...
    if (
      type === this.constants.RulesType.PROJECT ||
      type === this.constants.RulesType.SOW
    ) {
      dbItemList.map(
        (c) =>
          (c.csRuleArray =
            c.CSRule &&
            filterData.RULES.filter((d) =>
              c.CSRule.split(";#")
                .map((c) => +c)
                .includes(d.ID)
            )
    ...
    if (addedRuleArray.length > 0) {
      addedRuleArray = filterData.RULES.filter((c) =>
        addedRuleArray.map((i) => i.ID).includes(c.ID)
      );

      if (
        type === this.constants.RulesType.CD ||
        type === this.constants.RulesType.PF
      ) {
        dbItemList = await this.fetchProjectForRule(dbItemList);

        if (type === this.constants.RulesType.PF) {
          dbItemList = dbItemList.filter(
            (c) =>
              c.ProjectStatus !== this.constants.projectStatus.Cancelled &&
              c.ProjectStatus !== this.constants.projectStatus.Closed
          );
        }
      }
      if (type !== this.constants.RulesType.SOW) {
        dbItemList = await this.fetchProjectFinanceForRule(
          addedRuleArray,
          dbItemList
        );
      }

      dbItemList = await this.addRuleToItems(
        addedRuleArray,
        dbItemList,
        filterData,
        type,
      ...
      AddRuleItemsCount = dbItemList.filter((c) => c.edited === true)
        ? dbItemList.filter((c) => c.edited === true).length
        : 0;
    ...
    if (EditedRuleArray.length > 0) {
      ...
      const RemoveRuleArray = EditedRuleArray.filter(
        (c) => c.edited.IsActiveCH === true
      );
      if (RemoveRuleArray && RemoveRuleArray.length > 0) {
        this.commonService.showToastrMessage(
          this.constants.MessageType.info,
          "Removing " +
            RemoveRuleArray.length +
            " rules from" +
            disMessage +
            ".",
          true,
        ...
        RemoveRuleArray.forEach(async (oldrule) => {
          let RuleItems = [];
          const RuleTypeObj = this.constants.RuleTypeParameterArray.find(
            (c) => c.label === type
          )
            ? this.constants.RuleTypeParameterArray.find(
                (c) => c.label === type
              ).value.find((d) => d.label === oldrule.ResourceType)
            : "";
          if (RuleTypeObj) {
            RuleItems = dbItemList.filter(
              (c) =>
                c[RuleTypeObj.DBParameter] &&
                c[RuleTypeObj.DBParameter]
                  .split(";#")
                  .map((c) => +c)
                  .includes(oldrule.ID)
      ...
      const UpdatedUserRuleArray = EditedRuleArray.filter(
        (c) => c.edited.UserEdited === true
      );
      if (UpdatedUserRuleArray && UpdatedUserRuleArray.length > 0) {
        const dbUpdatedUserRules = filterData.RULES.filter((c) =>
          UpdatedUserRuleArray.map((d) => d.ID).includes(c.ID)
        );
        let ListOfProjects = [];
        dbUpdatedUserRules.forEach(async (updatedrule) => {
          let RuleItems = [];
          const oldRule = UpdatedUserRuleArray.find(
            (c) => c.ID === updatedrule.ID
          );
          const RuleTypeObj = this.constants.RuleTypeParameterArray.find(
            (c) => c.label === type
          )
            ? this.constants.RuleTypeParameterArray.find(
                (c) => c.label === type
              ).value.find((d) => d.label === updatedrule.ResourceType)
            : "";
          if (RuleTypeObj) {
            RuleItems = this.getItemCodes(
              RuleTypeObj.listName,
              dbItemList,
              oldRule,
              parameter
      ...
      if (type !== this.constants.RulesType.PF) {
        const sequenceUpdateRules = EditedRuleArray.filter(
          (c) => c.edited.IsActiveCH === false && c.edited.UserEdited === false
        );
        if (sequenceUpdateRules && sequenceUpdateRules.length > 0) {
          const dbsequenceUpdateRules = filterData.RULES.filter((c) =>
            sequenceUpdateRules.map((d) => d.ID).includes(c.ID)
          );
          dbsequenceUpdateRules.forEach(async (rule) => {
            const RuleTypeObj = this.constants.RuleTypeParameterArray.find(
              ...
              ? this.constants.RuleTypeParameterArray.find(
                  (c) => c.label === type
                ).value.find((d) => d.label === rule.ResourceType)
              : "";
            if (RuleTypeObj) {
  ...
  getItemCodes(ruleArray, dbItemList, rule, parameter) {
    ...
    return dbItemList.filter((c) => c[ruleArray].find((d) => d.ID === rule.ID))
      ? dbItemList
          .filter((c) => c[ruleArray].find((d) => d.ID === rule.ID))
          .map((e) => e[parameter])
      : [];
  }
  ...
  async updateAllEditedItems(ListOfUpdatedItems, TypeName, disMessage) {
    ...
    for (var i = 0; i < ListOfUpdatedItems.length; i++) {
      const data = {
        ...
        // ID: item.ID,
        DeliveryRule: ListOfUpdatedItems[i].deliveryRuleArray
          ? ListOfUpdatedItems[i].deliveryRuleArray.map((c) => c.ID).join(";#")
          : "",
      };

      if (
        TypeName === this.constants.RulesType.PROJECT ||
        TypeName === this.constants.RulesType.SOW
      ) {
        let allUsers = [];
        data["CSRule"] = ListOfUpdatedItems[i].csRuleArray
          ? ListOfUpdatedItems[i].csRuleArray.map((c) => c.ID).join(";#")
          : "";
        data["CMLevel1Id"] =
          ListOfUpdatedItems[i].CMLevel1 &&
          ListOfUpdatedItems[i].CMLevel1.hasOwnProperty("results") &&
          ListOfUpdatedItems[i].CMLevel1.results.length
            ? {
                results: ListOfUpdatedItems[i].CMLevel1.results.map(
                  (c) => c.ID
                ),
              }
            : { results: [] };
        data["CMLevel2Id"] = ListOfUpdatedItems[i].CMLevel2
          ? ListOfUpdatedItems[i].CMLevel2.ID
          : -1;
        data["DeliveryLevel1Id"] =
          ListOfUpdatedItems[i].DeliveryLevel1 &&
          ListOfUpdatedItems[i].DeliveryLevel1.hasOwnProperty("results") &&
          ListOfUpdatedItems[i].DeliveryLevel1.results.length
            ? {
                results: ListOfUpdatedItems[i].DeliveryLevel1.results.map(
                  (c) => c.ID
                ),
              }
            : { results: [] };
        data["DeliveryLevel2Id"] = ListOfUpdatedItems[i].DeliveryLevel2
          ? ListOfUpdatedItems[i].DeliveryLevel2.ID
          : -1;
        allUsers = allUsers.concat(
          data["CMLevel1Id"].results,
          [data["CMLevel2Id"]],
          data["DeliveryLevel1Id"].results,
          [data["DeliveryLevel2Id"]]
        );

        allUsers = allUsers.filter((item) => item !== -1);

        if (TypeName === this.constants.RulesType.PROJECT) {
        ...
        data["TLId"] =
          ListOfUpdatedItems[i].TL &&
          ListOfUpdatedItems[i].TL.hasOwnProperty("results") &&
          ListOfUpdatedItems[i].TL.results.length
            ? { results: ListOfUpdatedItems[i].TL.results.map((c) => c.ID) }
            : { results: [] };
        data["ASDId"] = ListOfUpdatedItems[i].ASD
          ? { results: [ListOfUpdatedItems[i].ASD.ID] }
          : { results: [] };
        data["CSId"] = ListOfUpdatedItems[i].CSId
          ? { results: ListOfUpdatedItems[i].CSId.results }
          : { results: [] };
      } else {
        data["DeliveryLeadsId"] =
          ListOfUpdatedItems[i].DeliveryLeads &&
          ListOfUpdatedItems[i].DeliveryLeads.hasOwnProperty("results") &&
          ListOfUpdatedItems[i].DeliveryLeads.results.length
            ? {
                results: ListOfUpdatedItems[i].DeliveryLeads.results.map(
                  (c) => c.ID
                ),
              }
            : { results: [] };
      }
      let url = this.spServices.getItemURL(ListName, +ListOfUpdatedItems[i].ID);
      this.commonService.setBatchObject(
        batchURL,
        url,
        data,
  ...
  async fetchProjectForRule(dbItemList) {
    ...
    ProjectCodes = dbItemList.map((c) => c.Title)
      ? [...new Set(dbItemList.map((c) => c.Title))]
      : [];
    if (ProjectCodes && ProjectCodes.length > 0) {
      ...
      if (batchResults && batchResults.length > 0) {
        ProjectInformations = batchResults.filter(
    ...
    if (ProjectInformations) {
      ...
      dbItemList.map(
        (c) =>
          (c.BusinessVertical = ProjectInformations.find(
            (d) => d.ProjectCode === c.Title
          )
            ? ProjectInformations.find((d) => d.ProjectCode === c.Title)
                .BusinessVertical
            : "")
      );
      ...
      dbItemList.map(
        (c) =>
          (c.ClientLegalEntity = ProjectInformations.find(
            (d) => d.ProjectCode === c.Title
          )
            ? ProjectInformations.find((d) => d.ProjectCode === c.Title)
                .ClientLegalEntity
      ...
      dbItemList.map(
        (c) =>
          (c.SubDivision = ProjectInformations.find(
            (d) => d.ProjectCode === c.Title
          )
            ? ProjectInformations.find((d) => d.ProjectCode === c.Title)
                .SubDivision
      ...
      dbItemList.map(
        (c) =>
          (c.DeliverableType = ProjectInformations.find(
            (d) => d.ProjectCode === c.Title
          )
            ? ProjectInformations.find((d) => d.ProjectCode === c.Title)
                .DeliverableType
      ...
      dbItemList.map(
        (c) =>
          (c.ProjectStatus = ProjectInformations.find(
            (d) => d.ProjectCode === c.Title
          )
            ? ProjectInformations.find((d) => d.ProjectCode === c.Title).Status
            : "")
      );
    }
    return dbItemList;
  ...
  async fetchProjectFinanceForRule(addedRuleArray, dbItemList) {
    ...
    let CurrencyArray = addedRuleArray.filter(
      (c) =>
        c.DisplayRules.filter((c) => c.InternalName === "Currency").length > 0
    )
      ? addedRuleArray
          .filter(
            (c) =>
              c.DisplayRules.filter((c) => c.InternalName === "Currency")
                .length > 0
          )
          .map((c) => c.DisplayRules.find((c) => c.InternalName === "Currency"))
          .map((c) => c.Value)
      : [];
    ...
    if (CurrencyArray.length > 0) {
      ...
      if (batchResults && batchResults.length > 0) {
        ProjectFinance = batchResults.filter(
          (c) => c.listName === this.constants.listNames.ProjectFinances.name
        )
          ? [
              ...new Set(
                batchResults
    ...
    if (ProjectFinance) {
      dbItemList.map(
        (c) =>
          (c.Currency = ProjectFinance.find((d) => d.Title === c.ProjectCode)
            ? ProjectFinance.find((d) => d.Title === c.ProjectCode).Currency
            : "")
      );
    }
    return dbItemList;
  ...
  async addRuleToItems(
    ...
    dbItemList.map(
      (c) =>
        (c.Bucket = filterData.CLE.find((d) => d.value === c.ClientLegalEntity)
          ? filterData.CLE.find((d) => d.value === c.ClientLegalEntity).Bucket
          : "")
    );
    addedRuleArray.forEach(async (Rule) => {
      ...
      Rule.DisplayRules.forEach((element) => {
        if (element.InternalName === "Title") {
          ...
          allItems = allItems.filter(
            (c) => c[element.InternalName] === element.Value
          );
        }
      });
      ...
      if (AllCodeList) {
        if (
          dbItemList.filter((c) => AllCodeList.includes(c[parameter])) &&
        ...
        const RuleTypeObj = this.constants.RuleTypeParameterArray.find(
          ...
          : "";
        if (RuleTypeObj) {
          dbItemList = await this.applyRuleToItem(
            RuleTypeObj.listName,
            Rule,
            dbItemList,
            AllCodeList,
  ...
  applyRuleToItem(
      ...
      dbItemList
        ...
        .map((c) =>
          ...
          c[access].results.length
            ? c[access].results.push(Rule.OwnerPG)
            : (c[access] = { results: [Rule.OwnerPG] })
        );
    }
    ...
    dbItemList
      .filter((c) => AllCodeList.includes(c[parameter]))
      .filter((c) => c[access] && c[access].hasOwnProperty("results"))
      .map(
        (c) =>
          (c[access].results = Array.from(
            new Set(c[access].results.map((a) => a.ID))
          ).map((id) => {
  ...
  UpdateItemsForRule(
    ...
    if (
      ...
      dbItemList.filter((c) => RuleItems.includes(c[parameter])).length > 0
    ) {
      dbItemList
      ...
      if (oldrule.Access && oldrule.Access.results) {
          ...
          .map(
            (c) =>
              (c[access].results = c[access].results
                ? c[access].results.filter(
                    (e) =>
                      !oldrule.Access.results.map((f) => f.ID).includes(e.ID)
                  )
                : [])
          );
        ...
        if (type == this.constants.RulesType.CD) {
          dbItemList
            ...
            .map(
              (c) =>
                (c["CSId"].results = c["CSId"].results
                  ? c["CSId"].results.filter(
                      (e) =>
                        !oldrule.Access.results.map((f) => f.ID).includes(e.ID)
                    )
                  : [])
        ...
        dbItemList
          ...
          .map(
            (d) =>
              (d["CSId"].results =
                d[arrayName] &&
                d[arrayName].length > 0 &&
                d[arrayName].filter((e) => e.Access.hasOwnProperty("results"))
                  ? [
                      ...new Set([
                        ...d["CSId"].results,
                        ...[
      ...
      if (type !== this.constants.RulesType.PF) {
        ...
        dbItemList
          ...
          .map(
            (d) =>
              (d[owner] =
                d[arrayName] && d[arrayName].length > 0
        ...
        dbItemList
          ...
          .map(
            (c) =>
              (c[access].results = c[access].results
                ? c[access].results.filter((e) => e.ID !== oldrule.OwnerPG.ID)
                : [])
          );
              ...
              (d[access].results =
                d[arrayName] && d[arrayName].length > 0
                  ? [
                      ...new Set([
                        ...d[access].results,
                        ...[...new Set(d[arrayName].map((f) => f.OwnerPG))],
                      ]),
                    ]
                  : d[access].results)
          );
  ...
  changeOwnerOfItems(
    ...
    if (
      ...
      dbItemList
        .filter((c) => c[arrayName].filter((c) => c.Id === rule.ID))
        .map((d) => (d.edited = true));
    }
    // list of project code / Sow code containing rule
    const RuleItems = dbItemList.filter((c) =>
      c[arrayName].find((d) => d.ID === rule.ID)
    )
      ? dbItemList
          .filter((c) => c[arrayName].find((d) => d.ID === rule.ID))
          .map((e) => e[parameter])
      : [];
