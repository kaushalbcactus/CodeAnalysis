...
export class ClientMasterdataComponent implements OnInit {
  ...
  constructor(
    ...
    if (
      this.adminConstants.toastMsg.SPMAA ||
      this.adminConstants.toastMsg.SPMAD ||
      this.adminConstants.toastMsg.EAPA
    ) {
      setTimeout(() => {
        this.common.showToastrMessage(
          this.constantsService.MessageType.warn,
          "You don'\t have permission,please contact SP Team.",
          true
        );
  ...
  async loadClientTable() {
    ...
    if (this.globalObject.userInfo.Groups.results.length) {
      const groups = this.globalObject.userInfo.Groups.results.map(
        (x) => x.LoginName
      );
      ...
      if (
        ...
        getClientLegalInfo = Object.assign(
          {},
          this.adminConstants.QUERY.GET_ACCESS_CLIENT_LEGAL_ENTITY_BY_ACTIVE
        );
        getClientLegalInfo.filter = getClientLegalInfo.filter
          .replace(/{{isActive}}/gi, this.adminConstants.LOGICAL_FIELD.YES)
          .replace(/{{UserId}}/gi, this.globalObject.currentUser.userId);
      }
    }
    ...
    const results = await this.spServices.readItems(
      this.constantsService.listNames.ClientLegalEntity.name,
      getClientLegalInfo
    );
  ...
  async loadRecentRecords(ID, isUpdate) {
    const resGet = Object.assign(
      {},
      this.adminConstants.QUERY.GET_CLIENT_LEGAL_ENTITY_BY_ID
    );
    resGet.filter = resGet.filter
      .replace(/{{isActive}}/gi, this.adminConstants.LOGICAL_FIELD.YES)
      .replace(/{{Id}}/gi, ID);
    this.common.SetNewrelic("admin", "admin-clientMaster", "GetCLE");
    ...
    if (result && result.length) {
      ...
      if (isUpdate) {
        const index = this.clientMasterDataRows.findIndex(
          (x) => x.ID === obj.ID
        );
    ...
    this.items = [
      {
        label: "Edit",
        command: (e) =>
          this.addEditClentLegalEntity("Edit Client Legal Entity", data),
      },
      ...
      .then(async (Confirmation) => {
        if (Confirmation === "Yes") {
          ...
          this.confirmUpdate(
            ...
            updateData,
            this.constantsService.listNames.ClientLegalEntity.name,
            this.constantsService.listNames.ClientLegalEntity.type,
            this.adminConstants.DELETE_LIST_ITEM.CLIENT_LEGAL_ENTITY
          );
        }
      });
  }
    ...
    switch (itemName) {
      case this.adminConstants.DELETE_LIST_ITEM.CLIENT_LEGAL_ENTITY:
        this.common.showToastrMessage(
          ...
          "The client legal entity " +
            data.ClientLegalEntity +
            " has deleted successfully.",
          true
        );
        const clientIndex = this.clientMasterDataRows.findIndex(
          (x) => x.ID === data.ID
        );
      ...
      case this.adminConstants.DELETE_LIST_ITEM.SUB_DIVISION:
        this.common.showToastrMessage(
          this.constantsService.MessageType.success,
          "The sub division " + data.SubDivision + " has deleted successfully.",
          true
        );
        const subDivisionindex = this.subDivisionDetailsRows.findIndex(
          (x) => x.ID === data.ID
        );
      ...
      case this.adminConstants.DELETE_LIST_ITEM.PURCHASE_ORDER:
        this.common.showToastrMessage(
          this.constantsService.MessageType.success,
          "The po " + data.PoNumber + " has deleted successfully.",
          true
        );
  ...
  async showSubDivision() {
    ...
    const getSubDivisionInfo = Object.assign(
      {},
      this.adminConstants.QUERY.GET_SUB_DIVISION_BY_ACTIVE
    );
    ...
    const results = await this.spServices.readItems(
      this.constantsService.listNames.ClientSubdivision.name,
      getSubDivisionInfo
    );
    if (results && results.length) {
      results.forEach((item) => {
  ...
  subDivisionMenu(data) {
    ...
    this.subDivisionItems.push({
      label: "Edit",
      command: (e) => this.addEditSubDivision("Edit SubDivision", data),
    });
    if (this.isUserSPMCA) {
      this.subDivisionItems.push({
        label: "Delete",
        command: (e) => this.deleteSubDivision(),
      });
    }
  }
  /**
    ...
    this.common
      ...
      .then(async (Confirmation) => {
        if (Confirmation === "Yes") {
          ...
          this.confirmUpdate(
            this.currSubDivisionObj,
            updateData,
            this.constantsService.listNames.ClientSubdivision.name,
            this.constantsService.listNames.ClientSubdivision.type,
            this.adminConstants.DELETE_LIST_ITEM.SUB_DIVISION
          );
        }
      });
  }
  ...
  async showPOC() {
    ...
    const getPocInfo = Object.assign(
      {},
      this.adminConstants.QUERY.GET_POC_BY_ACTIVE
    );
    ...
    const results = await this.spServices.readItems(
      this.constantsService.listNames.ProjectContacts.name,
      getPocInfo
    );
    if (results && results.length) {
      results.forEach((item) => {
        ...
        obj.RelationshipStrength = item.RelationshipStrength
      ...
      .then(async (Confirmation) => {
        if (Confirmation === "Yes") {
          ...
          this.confirmUpdate(
            ...
            updateData,
            this.constantsService.listNames.ProjectContacts.name,
            this.constantsService.listNames.ProjectContacts.type,
            this.adminConstants.DELETE_LIST_ITEM.POINT_OF_CONTACT
          );
        }
      });
  }
  ...
  getPOBudgetBreakUPData(poResult, poDetails) {
    const data: any = {
      ...
      Currency: this.showeditPO
        ? this.currPOObj.Currency
        : poDetails.value.currency,
  ...
  poMenu(data) {
    ...
    if (this.isUserSPMCA || this.isUserPO) {
      this.poItems = [
        {
          label: "Change Budget",
          command: (e) => this.changeBudgetDialog(data),
        },
        {
          label: "Edit",
          command: (e) => this.addEditPO("Edit Purchase Order", data),
        },
      ...
      .then(async (Confirmation) => {
        if (Confirmation === "Yes") {
          ...
          this.confirmUpdate(
            ...
            this.adminConstants.DELETE_LIST_ITEM.PURCHASE_ORDER
          );
        }
      });
  }
  ...
  async saveClient(clientDetails) {
    ...
    if (!this.showEditClient) {
      ...
      if (!results.hasOwnProperty("hasError") && !results.hasError) {
        ...
        if (
          !this.adminObject.dropdown.ClientGroupArray.some(
            (a) =>
              a.label &&
              a.label.toLowerCase() === clientDetails.value.group.toLowerCase()
          )
        ) {
          ...
          const newClientGroup = await this.spServices.createItem(
            this.constantsService.listNames.ClientGroup.name,
            clientGroupdata,
            this.constantsService.listNames.ClientGroup.type
          );

          this.adminObject.dropdown.ClientGroupArray.push({
            label: clientDetails.value.group,
            value: clientDetails.value.group,
          });
        }
  ...
  async createCLEMapping(clientDetails) {
    const data = {
      Title: !this.showEditClient
        ? clientDetails.value.name
        : this.currClientObj.ClientLegalEntity,
      CLEName: !this.showEditClient
        ? clientDetails.value.name
        : this.currClientObj.ClientLegalEntity,
      Bucket: clientDetails.value.bucket,
      StartDate: !this.showEditClient
        ? new Date()
        : clientDetails.value.bucketEffectiveDate,
    };
    this.common.SetNewrelic(
      "admin",
      "admin-clientMaster",
      "CreateCLEBucketMapping"
    );
    const results = await this.spServices.createItem(
      this.constantsService.listNames.CLEBucketMapping.name,
      data,
      this.constantsService.listNames.CLEBucketMapping.type
    );
  }
  /**
  ...
  async updateCLEMapping(clientDetails) {
    const cleMappingGet = Object.assign(
      {},
      this.adminConstants.QUERY.GET_CLE_MAPPING_BY_ID
    );
    cleMappingGet.filter = cleMappingGet.filter.replace(
    ...
    if (result && result.length) {
      ...
      const updateItem = {
        EndDateDT: new Date(
          new Date(tempDate).setDate(new Date(tempDate).getDate() - 1)
        ),
      };
      this.common.SetNewrelic(
        "admin",
        "admin-clientMaster",
        "UpdateCLEBucketMapping"
      );
  ...
  getClientData(clientDetails) {
    ...
    if (!this.showEditClient) {
      (data.ListName = clientDetails.value.name
        ? clientDetails.value.name.replace(/[^a-zA-Z]/g, "").substring(0, 20)
        : ""),
    ...
    data.DistributionList = clientDetails.value.distributionList
      ? clientDetails.value.distributionList
      : "";
    ...
    data.APEmail = clientDetails.value.APEmail
      ? clientDetails.value.APEmail
      : "";
    data.NotesMT = clientDetails.value.notes ? clientDetails.value.notes : "";
    const ap1 = clientDetails.value.address1
      ? clientDetails.value.address1
      : "";
    const ap2 = clientDetails.value.address2
      ? clientDetails.value.address2
      : "";
    const ap3 = clientDetails.value.address3
      ? clientDetails.value.address3
      : "";
    const ap4 = clientDetails.value.address4
      ? clientDetails.value.address4
      : "";
    ...
    if (!this.showeditSubDivision) {
      ...
      if (!results.hasOwnProperty("hasError") && !results.hasError) {
        this.common.showToastrMessage(
          this.constantsService.MessageType.success,
          "The subdivision " +
            subDivisionDetails.value.subDivision_Name +
            " is created successfully.",
          false
        );
        await this.loadRecentSubDivisionRecords(
          results.ID,
          this.showeditSubDivision
        );
      }
    }
    if (this.showeditSubDivision) {
      this.common.SetNewrelic(
        "admin",
        "admin-clientMaster",
        "updateClientSubdivision"
      );
      const results = await this.spServices.updateItem(
        this.constantsService.listNames.ClientSubdivision.name,
        this.currSubDivisionObj.ID,
        subDivisionData,
        this.constantsService.listNames.ClientSubdivision.type
      );
      this.common.showToastrMessage(
        this.constantsService.MessageType.success,
        "The subdivision " +
          this.currSubDivisionObj.SubDivision +
          " is updated successfully.",
        false
      );
      await this.loadRecentSubDivisionRecords(
  ...
  getSubDivisionData(subDivisionDetails) {
    ...
    data.DeliveryLevel1Id = subDivisionDetails.value.deliveryLevel1
      ? { results: subDivisionDetails.value.deliveryLevel1 }
      : { results: [] };
    data.CMLevel1Id = subDivisionDetails.value.cmLevel1
      ? { results: subDivisionDetails.value.cmLevel1 }
      : { results: [] };
    data.DistributionList = subDivisionDetails.value.distributionList
      ? subDivisionDetails.value.distributionList
      : "";
    return data;
  }
  ...
  async loadRecentSubDivisionRecords(ID, isUpdate) {
    const subDivisionGet = Object.assign(
      {},
      this.adminConstants.QUERY.GET_SUB_DIVISION_BY_ID
    );
    ...
    if (result && result.length) {
      ...
      // If Edit - Replace the item in the array and position at 0 in the array.
      if (isUpdate) {
        const index = this.subDivisionDetailsRows.findIndex(
          (x) => x.ID === obj.ID
        );
    ...
    if (!this.showeditPOC) {
      ...
      if (!results.hasOwnProperty("hasError") && !results.hasError) {
        this.common.showToastrMessage(
          this.constantsService.MessageType.success,
          "The Poc " +
            pocDetails.value.fname +
            " " +
            pocDetails.value.lname +
            " is created successfully.",
          false
        );
  ...
  getPOCData(pocDetails) {
    ...
    data.DepartmentST = pocDetails.value.department
      ? pocDetails.value.department
      : "";
    data.RelationshipStrength = pocDetails.value.relationshipStrength
      ? pocDetails.value.relationshipStrength
      : "";
    data.EngagementPlan = pocDetails.value.engagementPlan
      ? pocDetails.value.engagementPlan
      : "";
    data.CommentsMT = pocDetails.value.comments
      ? pocDetails.value.comments
      : "";
    return data;
  }
  ...
  async confirmBudgetUpdate() {
    ...
    this.adminObject.finalBudget.AmountRevenue =
      this.adminObject.oldBudget.AmountRevenue +
      this.adminObject.newBudget.AmountRevenue;
    this.adminObject.finalBudget.AmountOOP =
      this.adminObject.oldBudget.AmountOOP +
      this.adminObject.newBudget.AmountOOP;
    this.adminObject.finalBudget.AmountTax =
      this.adminObject.oldBudget.AmountTax +
      this.adminObject.newBudget.AmountTax;
    const poData = {
    ...
    const poBudgetBreakupData = {
      __metadata: {
        type: this.constantsService.listNames.POBudgetBreakup.type,
      },
    ...
    updatePOData.url = this.spServices.getItemURL(
      this.constantsService.listNames.PO.name,
      this.currPOObj.ID
    );
    ...
    createPOBudgetBreakupObj.url = this.spServices.getReadURL(
      this.constantsService.listNames.POBudgetBreakup.name,
      null
    );
