...
export class BucketMasterdataComponent implements OnInit {
  ...
  async loadBucketTable() {
    ...
    if (results && results.length) {
      ...
      if (results[1].retItems.hasError) {
        this.common.showToastrMessage(
          ...
          results[1].retItems.message.value,
          false
        );
        return false;
      }
      ...
      this.bucketArray.forEach((item) => {
        ...
        const clientFilteredArray = this.clientArray.filter(
          (x) => x.Bucket === item.Title
        );
        ...
        obj.Client =
          clientFilteredArray && clientFilteredArray.length
            ? clientFilteredArray.map((x) => x.Title).join(", ")
            : "";
    ...
    this.bucketDataColumns = this.common.MainfilterForTable(
      this.bucketDataColumns,
      this.bucketDataRows
    );
  }
  ...
  async getInitData() {
    ...
    const bucketFilter = Object.assign(
      {},
      this.adminConstants.QUERY.GET_FOCUS_GROUP_BY_ACTIVE
    );
    bucketFilter.filter = bucketFilter.filter.replace(
      /{{isActive}}/gi,
      this.adminConstants.LOGICAL_FIELD.YES
    );
    bucketGet.url = this.spServices.getReadURL(
      this.constants.listNames.FocusGroup.name,
      bucketFilter
    );
    bucketGet.type = "GET";
    ...
    const clientLegalEntityFilter = Object.assign(
      {},
      this.adminConstants.QUERY.GET_CLIENTLEGALENTITY_BY_ACTIVE
    );
    clientLegalEntityFilter.filter = clientLegalEntityFilter.filter.replace(
      /{{isActive}}/gi,
      this.adminConstants.LOGICAL_FIELD.YES
    );
    getClientLegalEntity.url = this.spServices.getReadURL(
    ...
    this.common.SetNewrelic(
      "admin",
      "admin-attribute-bucketMasterData",
      "GetFocusDataCLE"
    );
        ...
        clientArray.find((item, index) => {
          if (item.Title === element.CLEName) {
            clientArray[index]["EffectiveDate"] = new Date(
              this.datepipe.transform(element.StartDate, "MMM dd yyyy")
            );
          } else {
            clientArray[index]["EffectiveDate"] = new Date();
          }
  ...
  async getCleBMList(item) {
    ...
    const cleFilter = Object.assign(
      {},
      this.adminConstants.QUERY.GET_BUCKET_MAPPING_BY_ID
    );
    cleFilter.filter = cleFilter.filter.replace(/{{bucket}}/gi, item);
    cleMappingGet.url = this.spServices.getReadURL(
      this.constants.listNames.CLEBucketMapping.name,
      cleFilter
    );
  ...
  bucketMenu(data) {
    ...
    this.items = [
      ...
      {
        label: "Delete",
        command: (e) => this.delete(data),
        visible: data.Client !== "" ? false : true,
      },
    ];
  }
  /**
   ...
   */
  async addBucketData() {
    const alphaExp = new RegExp(
      this.adminConstants.REG_EXPRESSION.ALPHA_SPECIAL
    );
    ...
    if (
      this.bucketDataRows.some(
        (a) => a.Bucket.toLowerCase() === this.bucketData.toLowerCase()
      )
    ) {
      this.common.showToastrMessage(
        this.constants.MessageType.error,
        "This bucket is already exist. Please enter another bucket name.",
        false
      );
      return false;
    }
    ...
    this.common.showToastrMessage(
      ...
      "The bucket " + this.bucketData + " has added successfully.",
      true
    );
    this.bucketData = "";
  ...
  async confirmUpdate(data, updateData, listName, type) {
    ...
    this.common.showToastrMessage(
      this.constants.MessageType.success,
      "The bucket " + data.Bucket + " has deleted successfully.",
      true
    );
    await this.loadBucketTable();
    this.loaderenable = false;
  ...
  async update() {
    ...
    this.selectedClient.forEach((element) => {
      if (!element.isCheckboxDisabled) {
        ...
        updateClientData.url = this.spServices.getItemURL(
          this.constants.listNames.ClientLegalEntity.name,
          element.ID
        );
    ...
    if (batchURL.length) {
      ...
      this.common.showToastrMessage(
        this.constants.MessageType.success,
        "The selected clients are updated successfully.",
        true
      );
     await this.loadBucketTable();
    }
  ...
  async createCLEMapping(batchURL) {
    ...
    this.selectedClient.forEach((element) => {
      if (!element.isCheckboxDisabled) {
        ...
        createCleMapping.url = this.spServices.getReadURL(
          this.constants.listNames.CLEBucketMapping.name,
          null
        );
  ...
  async updateCLEMapping(batchURL) {
    ...
    this.selectedClient.forEach((element) => {
      if (!element.isCheckboxDisabled) {
        ...
        cleFilter.filter = cleFilter.filter.replace(
      ...
      if (getCLEResults && getCLEResults.length) {
        getCLEResults.forEach((element) => {
          if (element.retItems && element.retItems.length) {
            ...
            const tempDate = this.selectedClient.filter(
              (a) => a.Title === cleObj.CLEName
            );
            const updateData = {
              __metadata: {
                type: this.constants.listNames.CLEBucketMapping.type,
              },
              EndDateDT: new Date(
                new Date(tempDate[0].EffectiveDate).setDate(
                  new Date(tempDate[0].EffectiveDate).getDate() - 1
                )
              ),
            };
            ...
            updateCleMapping.url = this.spServices.getItemURL(
              this.constants.listNames.CLEBucketMapping.name,
              cleObj.ID
            );
