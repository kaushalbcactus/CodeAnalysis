    ...
    for (const userIndex in selectedUsers) {
      if (selectedUsers.hasOwnProperty(userIndex)) {
        if (selectedUsers[userIndex]) {
          ...
          const userDetail = this.sharedObject.oTaskAllocation.oResources.filter((objt) => {
          ...
          if (userDetail.length > 0) {
            ...
            const userCapacityCall = Object.assign({}, this.taskAllocationService.taskallocationComponent.userCapacity);
            userCapacityCall.filter = userCapacityCall.filter.replace(/{{selectedUserID}}/gi, oUser.uid)
            .replace(/{{startDateString}}/gi, startDateString).replace(/{{endDateString}}/gi, endDateString);
    ...
    for (const indexUser in this.oCapacity.arrUserDetails) {
      if (this.oCapacity.arrUserDetails.hasOwnProperty(indexUser)) {
        ...
        if (this.oCapacity.arrUserDetails[indexUser].tasks.length) {
          this.oCapacity.arrUserDetails[indexUser].tasks.sort((a, b) => {
        ...
        for (const j in oUser.tasks) {
          if (oUser.tasks.hasOwnProperty(j)) {
            ...
            if (oUser.tasks[j].Task !== 'Adhoc') {
              ...
              oUser.tasks[j].timeAllocatedPerDay = oUser.tasks[j].TimeSpent !== null ? ''
              + oUser.tasks[j].TimeSpent.replace('.', ':') : '0:0';
            }
        ...
        if (users.hasOwnProperty(i)) {
          ...
          for (const j in users[i].tasks) {
            if (users[i].tasks.hasOwnProperty(j)) {
              ...
              let dateIndex = this.oCapacity.arrUserDetails[0].businessDays.findIndex(x => {
