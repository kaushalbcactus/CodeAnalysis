...
export class DragDropComponent implements OnInit {
              ...
              element.children.forEach(element => {
                ...
                else
                {
                  if(temp.position !== undefined) {
                    this.tempSubmileArray.push(temp);
                  }
                  element.children.forEach(element => {
                    if(temp.status === undefined || temp.status !=='Not Confirmed' || temp.status !=='Not Started') {
                      temp.status = element.data.status
                    }
                  })
                  this.onDrop(temp,element.data.type);
  ...
  SaveGraph()
  {
    ...
    if( this.milestonesGraph.nodes.length > 0)
    {
      this.milestonesGraph.nodes.forEach(milestone => {
        milestone.submilestone.nodes.forEach(submilestone => {   
          if(submilestone.task.nodes.length  === 0)
          {
            if(errorM <= 0)
            {
              ...
              {
                this.messageService.add({key: 'custom', severity:'warn', summary: 'Warning Message', detail:'Please click on Milestone to add Sub Milestone/Tasks.'});
                return false;
              }
              else if(this.submilestoneIndex === -1)
              {
                this.messageService.add({key: 'custom', severity:'warn', summary: 'Warning Message', detail:"Please click on "+ submilestone.label+" Sub Milestone to add Tasks."});
                return false;
              }
              else
              {
                this.messageService.add({key: 'custom', severity:'warn', summary: 'Warning Message', detail:'Please add Tasks in '+ submilestone.label+ ' of '+milestone.label+  '.'});
                return false;
              }
            } 
          } 
          else
          {
            ...
            ScPresent = ScPresent === false ? (submilestone.task.nodes.find(c=>c.taskType === 'Send to client') !== undefined ? true : false) : true;

            // if(ScPresent === false && submilestone.label ==='Default' )
            // {
            //   return false;
            // }
            ...
            {
              errorM++;
              this.messageService.add({key: 'custom', severity:'warn', summary: 'Warning Message', detail:'Please add Client Review Task in '+ submilestone.label +' of '+ milestone.label  });
              return false;
            }
 
            if(milestone.submilestone.nodes.length > 1 && submilestone.label === 'Default' && submilestone.task.nodes.length > 1){
              errorM++;
              this.messageService.add({key: 'custom', severity:'warn', summary: 'Warning Message', detail:'Please remove Task in '+ submilestone.label +' of '+ milestone.label +' only Client Review is Required.'  });
              return false;
            }
            const tempnodes = submilestone.task.nodes.map(c => c.id).filter(c => !submilestone.task.links.map(c => c.source).includes(c) && !submilestone.task.links.map(c => c.target).includes(c)).filter(c=> !submilestone.task.nodes.filter(c => (c.taskType === 'Client Review' || c.taskType === 'Adhoc')).map(c=>c.id).includes(c));

            if(tempnodes.length > 0 && errorM === 0)
            {
              var missingLinkTasks = submilestone.task.nodes.filter(c=> tempnodes.includes(c.id)).map(c=> submilestone.label +" - " + c.label);
              errorM++;
              this.messageService.add({key: 'custom', severity:'warn', summary: 'Warning Message', detail:'Paths missing for following tasks '+missingLinkTasks+' of '+ milestone.label});
              return false;
            } 
          }
          ...
          if(submilestone.task.links.length > 0)
          {
            ...
            if(circularPresent) {
              errorM++;
              this.messageService.add({key: 'custom', severity:'warn', summary: 'Warning Message', detail:'Circular links present. Please delete the circular links in '+  submilestone.label+ '-'+ milestone.label });
              return false;
            }
          }
        });  
        ...
        {
          errorM++;
          this.messageService.add({key: 'custom', severity:'warn', summary: 'Warning Message', detail:'Please add Send to Client in '+ milestone.label });
          return false;
        }
        ScPresent = false;
      });
      ...
      if(errorM <= 0)
      {
        ...
        this.milestonesGraph.nodes.forEach(milestone => {
          ...
          if(milestone.submilestone.nodes.length > 1)
          {
              ...
              var index = milestone.submilestone.nodes.indexOf(DefaultObj);
              if(index > -1)
              {
          ...
          milestone.submilestone.nodes.forEach(submilestone => {
            submilestone.task.nodes.sort(function(a, b) { 
  ...
  getNextTarget(source, target, submilestone, currentPath, circularPresent) {
      ...
      let allPaths = currentPath.split(',');
  ...
  cancelGraph(){
    ...
    var result = confirm("Are you sure that you want to discard all changes?");
    if (result) {
      this.ref.close();
    }
  ...
  async GetAllTasksMilestones() {
    ...
    this.taskAllocationService.taskallocationComponent.submilestonesList.filter = this.taskAllocationService.taskallocationComponent.submilestonesList.filter.replace(/{{status}}/gi, 'Yes');
    const submilestoneListUrl = this.spServices.getReadURL('' + this.constants.listNames.SubMilestones.name + '', this.taskAllocationService.taskallocationComponent.submilestonesList);
  ...
  ontaskUp(event, node) {
    if (this.taskDown != null) {
      if (this.taskDown !== node) {
        ...
        if(this.taskDown.taskType !=='Client Review' && (this.taskDown.taskType !== 'Send to client' && this.taskUp.taskType !== 'Client Review' ))
        {
          var link = {
